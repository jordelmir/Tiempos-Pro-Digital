...
let DB_SETTINGS = load(DB_STORAGE_KEYS.SETTINGS, { multiplier_tiempos: 90, multiplier_reventados: 200 });
let DB_LIMITS = load(DB_STORAGE_KEYS.LIMITS, []);

export const MockDB = {
    getUsers: () => DB_USERS,
    saveUser: (user: any) => {
        const idx = DB_USERS.findIndex((u: any) => u.id === user.id);
        if (idx >= 0) DB_USERS[idx] = user; else DB_USERS.unshift(user);
    ...
    getBets: () => DB_BETS,
    addBet: (bet: any) => { DB_BETS.unshift(bet); save(DB_STORAGE_KEYS.BETS, DB_BETS); },
    getLedger: () => DB_LEDGER,
    ...
    saveResult: (res: any) => {
        ...
        const idx = DB_RESULTS.findIndex((r: any) => r.drawTime === res.drawTime && r.date === res.date);
        if (idx >= 0) {
    ...
    getAudit: () => DB_AUDIT,
    addAudit: (log: any) => {
        const newLog = { id: Date.now() + Math.random(), event_id: `EVT-${Date.now()}`, timestamp: new Date().toISOString(), hash: `sha-${Math.random()}`, ...log };
        DB_AUDIT.unshift(newLog);
        save(DB_STORAGE_KEYS.AUDIT, DB_AUDIT);
    },
    getSettings: () => DB_SETTINGS,
    saveSettings: (s: any) => { DB_SETTINGS = { ...DB_SETTINGS, ...s }; save(DB_STORAGE_KEYS.SETTINGS, DB_SETTINGS); },
    getSettingsList: () => [],
    updateSetting: (k: string, v: any) => {},
    getLimits: () => DB_LIMITS,
    saveLimit: (limit: any) => {
        const idx = DB_LIMITS.findIndex((l: any) => l.draw_type === limit.draw_type && l.number === limit.number);
        if (idx >= 0) DB_LIMITS[idx] = limit; else DB_LIMITS.push(limit);
    ...
    analyzePurge: (target: PurgeTarget, days: number): PurgeAnalysis => {
        ...
        if (target === 'BETS_HISTORY') {
            list = DB_BETS.filter(b => b.status !== 'PENDING' && new Date(b.created_at).getTime() < cutoff);
            desc = "Registros de apuestas finalizadas (Ganadas/Perdidas). Las apuestas en curso NO se verán afectadas.";
            risk = "MEDIUM";
    ...
    executePurge: (target: PurgeTarget, days: number): number => {
        ...
        if (target === 'BETS_HISTORY') {
            ...
            DB_BETS = DB_BETS.filter(b => b.status === 'PENDING' || new Date(b.created_at).getTime() >= cutoff);
  ...
  client = {
    ...
    auth: {
      ...
      signInWithPassword: async ({ email }: any) => {
        ...
        let target = DB_USERS.find((u: any) => u.email === email) || MOCK_ADMIN_PROFILE;
