...
async function executeDirectDB<T>(fn: string, body: any, session: any): Promise<ApiResponse<T>> {
    ...
    const logAudit = async (action: string, type: string, severity: string, metadata: any = {}, target?: string) => {
    ...
    try {
        ...
        if (fn === 'placeBet') {
            ...
            await logAudit('PLACE_BET', 'GAME_BET', 'SUCCESS', { ticket: ticketCode, amount: body.amount, numbers: body.numbers }, ticketCode);
        ...
        if (fn === 'getGlobalSettings') {
            ...
            return { data: { multiplier_tiempos: Number(multT), multiplier_reventados: Number(multR) } as any };
        }
        ...
        // 4. USERS
        if (fn === 'rechargeUser') {
            const { data: user } = await supabase.from('app_users').select('*').eq('id', body.target_user_id).single();
            if (!user) return { error: 'Usuario no encontrado' };
            ...
            await supabase.from('ledger_transactions').insert({
                user_id: user.id, amount_bigint: body.amount, type: 'CREDIT', balance_before: user.balance_bigint, balance_after: newBal, reference_id: txId, meta: { desc: 'Recarga Manual' }
            });
            await logAudit('USER_RECHARGE', 'TX_DEPOSIT', 'SUCCESS', { amount: body.amount, newBalance: newBal }, user.id);
        ...
        if (fn === 'withdrawUser') {
            ...
            await supabase.from('ledger_transactions').insert({
                user_id: user.id, amount_bigint: -body.amount, type: 'DEBIT', balance_before: user.balance_bigint, balance_after: newBal, reference_id: txId, meta: { desc: 'Retiro Manual' }
            });
            await logAudit('USER_WITHDRAW', 'TX_WITHDRAWAL', 'SUCCESS', { amount: body.amount, newBalance: newBal }, user.id);
            return { data: { new_balance: newBal, tx_id: txId } as any };
        }
        ...
        if (fn === 'createUser') {
            ...
            const { data: issuer } = await supabase.from('app_users').select('id').eq('auth_uid', session.user.id).single();
            ...
            const { data: newUser, error: cErr } = await supabase.from('app_users').insert({
                name: body.name,
                cedula: body.cedula,
            ...
            // Audit Log
            await logAudit('CREATE_USER', 'IDENTITY_REGISTER', 'FORENSIC', { new_user: newUser.id, role: newUser.role }, newUser.id);
        ...
        if (fn === 'analyzePurge') {
            ...
            if (table) {
                const { count: c } = await supabase.from(table).select('*', { count: 'exact', head: true }).lt('created_at', cutoffStr);
                count = c || 0;
            }

            return { data: { recordCount: count, estimatedSizeKB: Math.round(count * 0.5), riskLevel: count > 1000 ? 'HIGH' : 'LOW', description: `Se encontraron ${count} registros antiguos.` } as any };
        }
        ...
        if (fn === 'executePurge') {
            ...
            if (table) {
                ...
                const { count: c } = await supabase.from(table).delete({ count: 'exact' }).lt(timeCol, cutoffStr);
                totalDeleted = c || 0;
            }

            await logAudit('PURGE_DATA', 'ADMIN_PURGE', 'CRITICAL', { target, deleted: totalDeleted, cutoff: cutoffStr }, 'SYSTEM');
        ...
        if (fn === 'updateUserStatus') {
            ...
            const { error } = await supabase.from('app_users').update({ status }).eq('id', target_user_id);
            if (error) return { error: 'Error actualizando estado' };
            await logAudit('UPDATE_USER_STATUS', 'SYSTEM_INTEGRITY', 'WARNING', { status }, target_user_id);
            return { data: { success: true } as any };
        }
        ...
        if (fn === 'deleteUser') {
            ...
            if (error) {
                ...
                const { error: softErr } = await supabase.from('app_users')
            ...
            await logAudit('DELETE_USER', 'SYSTEM_INTEGRITY', 'CRITICAL', { hard_delete: !error }, target_user_id);
            return { data: { success: true } as any };
        }
        ...
        if (fn === 'getGlobalBets') {
            ...
            if (userIds.length > 0) {
                try {
                    const { data } = await supabase.from('app_users').select('id, name, role').in('id', userIds);
                    usersData = data || [];
                } catch (uErr) { console.error("User fetch error:", uErr); }
            }
        ...
        if (fn === 'publishDrawResult') {
            const { date, drawTime, winningNumber, isReventado, reventadoNumber, actor_id } = body;

            // 1. Insert Draw Result
            const { error: drErr } = await supabase.from('draw_results').insert({
                date, "drawTime": drawTime, "winningNumber": winningNumber, "isReventado": isReventado, "reventadoNumber": reventadoNumber, status: 'CLOSED'
            });
                ...
                for (const bet of pendingBets) {
                    ...
                    if (isWin) {
                        ...
                        updates.push((async () => {
                            try {
                                // 1. Mark Bet WON
                                const { error: bErr } = await supabase.from('bets').update({ status: 'WON' }).eq('id', bet.id);
                                if (bErr) throw new Error(`Bet status update failed: ${bErr.message}`);

                                // 2. Update Balance & Ledger (User)
                                const { data: user, error: uFetchErr } = await supabase.from('app_users').select('balance_bigint').eq('id', bet.user_id).single();
                                if (uFetchErr || !user) throw new Error(`User fetch failed: ${uFetchErr?.message}`);

                                const newBalance = Number(user.balance_bigint) + prize;

                                const { error: uParamsErr } = await supabase.from('app_users').update({ balance_bigint: newBalance }).eq('id', bet.user_id);
                                ...
                                await supabase.from('ledger_transactions').insert({
                                    ...
                                    balance_after: newBalance,
                                    ticket_code: bet.ticket_code,
                                    meta: { draw: drawTime, number: winningNumber, description: 'Premio Automático' },
                                ...
                                await logAudit('PAYOUT_FAILURE', 'GAME_ERROR', 'CRITICAL', { betId: bet.id, error: e.message }, bet.ticket_code);
            ...
            await logAudit('PUBLISH_RESULTS', 'GAME_EVENT', 'CRITICAL', { draw: drawTime, winner: winningNumber, total_processed: processedCount }, `${date}:${drawTime}`);
