...
export default function WinningNumberCard({
  ...
  return (
    <div
      className={`relative group overflow-hidden rounded-3xl border-2 transition-all duration-500 hover:scale-[1.02] h-80 flex flex-col ${theme.cardBorder} ${theme.cardShadow}`}
      style={{ backgroundColor: theme.bgHex }}
    >
      <div
        className={`absolute -inset-1 ${isReventado ? 'bg-red-600' : theme.textColor.replace('text-', 'bg-')} opacity-5 blur-xl animate-pulse`}
      ></div>
      ...
      <div className="relative z-10 p-4 flex justify-between items-center border-b border-white/5 bg-black/40 backdrop-blur-sm">
        <div className="flex items-center gap-2">
          <AnimatedIconUltra
            profile={{ animation: 'spin3d', speed: 5, size: 0.85, theme: 'minimal' }}
          >
        ...
        <div
          className={`text-[8px] font-mono px-2 py-0.5 rounded border ${result?.status === 'CLOSED' ? 'border-green-900 text-green-500 bg-green-900/20' : 'border-slate-800 text-slate-600'}`}
        >
      ...
      <div className="relative z-10 flex-1 flex items-center justify-center overflow-hidden p-4">
        {isReventado ? (
          <div className="relative w-64 h-64 flex items-center justify-center">
            <div className="relative z-30 w-28 h-28 rounded-full bg-[#1a0000] border-4 border-red-600 shadow-[0_0_60px_#ff0000,inset_0_0_20px_rgba(255,0,0,0.5)] flex items-center justify-center animate-pulse">
              <div className="flex flex-col items-center">
                ...
                <span className="text-[8px] font-black text-red-400 uppercase tracking-widest bg-red-950/80 px-2 py-0.5 rounded mt-1 border border-red-500/30">
                  REV
                </span>
              </div>
            </div>
            ...
            <div className="absolute w-48 h-48 animate-[spin_7s_linear_infinite] z-40">
              <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-16">
                <div className="w-full h-full animate-[spin_7s_linear_infinite_reverse]">
                  <div
                    className={`w-full h-full rounded-full ${theme.ballBg} border-2 ${theme.borderColor} ${theme.shadow} flex items-center justify-center relative shadow-[0_0_30px_rgba(0,240,255,0.3)]`}
                  >
                    <div className="absolute top-1 left-2 w-4 h-2 bg-white opacity-20 blur-sm rounded-full"></div>
                    <span
                      className={`font-mono font-black text-xl drop-shadow-[0_0_10px_rgba(52,211,153,0.8)] ${isSpinning ? 'text-white blur-[1px]' : 'text-emerald-400'}`}
                    >
                      {displayNumber}
                    </span>
                  </div>
          ...
          >
            ...
            <span
              className={`font-mono font-black tracking-tighter text-7xl drop-shadow-[0_0_25px_rgba(16,185,129,0.7)] ${isSpinning ? 'blur-md scale-110 text-white' : 'scale-100 text-emerald-400'} transition-all duration-100`}
            >
              {displayNumber}
            </span>
          </div>
