...
export default function CountdownTimer({ role }: CountdownTimerProps) {
  ...
  return (
    <div
      className={`relative group h-full min-h-[180px] overflow-hidden rounded-3xl bg-[#02040a] border-2 transition-all duration-700 ${theme.border} ${theme.shadow}`}
    >
      {/* --- REACTOR CORE BACKLIGHT --- */}
      <div
        className={`absolute top-1/2 -translate-y-1/2 right-0 w-1/2 h-full opacity-20 blur-[60px] animate-pulse transition-colors duration-1000 ${theme.bg.replace('/40', '').replace('950', '600')}`}
      ></div>
      ...
      <div className="relative z-10 flex h-full p-6">
        ...
        <div className="flex-1 flex flex-col justify-between">
          ...
          <div>
            <div
              className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border bg-black/60 backdrop-blur-sm mb-4 transition-all duration-500 ${theme.border} ${theme.color}`}
            >
              <div
                className={`w-1.5 h-1.5 rounded-full ${status === 'WARNING' ? 'animate-ping' : 'animate-pulse'} bg-current`}
              ></div>
              <span className="text-[9px] font-black uppercase tracking-[0.2em] leading-none pt-0.5">
                {theme.label}
              </span>
            </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-3 flex items-center gap-3 backdrop-blur-sm">
            <div
              className={`w-8 h-8 rounded-lg bg-black flex items-center justify-center border border-white/10 ${theme.color}`}
            >
            ...
            <div>
              ...
              <div className="text-xs font-mono font-bold text-white flex items-center gap-2">
                ...
                {isOffline && (
                  <span className="text-[8px] bg-red-500 text-black px-1 rounded font-bold animate-pulse">
                    OFF
                  </span>
                )}
              </div>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center z-20">
            {loading ? (
              ...
              <div className="text-center">
                <div
                  className={`text-2xl font-mono font-black tracking-tighter leading-none ${theme.color} drop-shadow-[0_0_10px_currentColor]`}
                >
                  {timeObj.h}:{timeObj.m}
                </div>
                <div
                  className={`text-lg font-mono font-bold leading-none mt-1 opacity-80 ${theme.color}`}
                >
                  {timeObj.s}
                </div>
