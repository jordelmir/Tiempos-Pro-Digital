...
export default function AuditView() {
  ...
  // Move function definition above useEffect to avoid hoisting issues in strict linting
  ...
  useEffect(() => {
    // FIXED: Call async fetcher in a defer to avoid synchronous setState lint warning
    const init = async () => {
      await fetchLogs();
    };
    init();
  ...
  const exportLogs = () => {
    ...
    // Guide for Forensic Analysis
    const guide = [
      '# BITACORA FORENSE DIGITAL - TIEMPOS PRO',
      '# GUIA DE TRAZABILIDAD:',
      '# - SEVERITY: Nivel de criticidad (INFO = Rutina, WARNING = Alerta, CRITICAL = Incidente).',
      '# - ACTION: Acción específica ejecutada en el sistema.',
      '# - DETAILS: Descripción narrativa del evento.',
      '# - TIMESTAMP: Marca de tiempo exacta del suceso (UTC Localizado).',
      '# - IP_ADDRESS: Dirección de origen de la solicitud (Si está disponible).',
      '# ------------------------------------------------------------',
    ].join('\n');
  ...
  const getIcon = (type: AuditEventType) => {
    switch (type) {
      case AuditEventType.IDENTITY_REGISTER:
        return 'fa-user-plus';
      case AuditEventType.IDENTITY_COLLISION:
        return 'fa-users-slash';
      case AuditEventType.SESSION_LOGIN:
        return 'fa-sign-in-alt';
      case AuditEventType.TX_DEPOSIT:
        return 'fa-arrow-down';
      case AuditEventType.TX_WITHDRAWAL:
        return 'fa-arrow-up';
      case AuditEventType.ADMIN_PURGE:
        return 'fa-radiation';
      case AuditEventType.GAME_BET:
        return 'fa-ticket-alt';
      case AuditEventType.AI_OPERATION:
        return 'fa-brain';
      case AuditEventType.SYSTEM_INTEGRITY:
  ...
  const getIconBoxStyle = (sev: AuditSeverity) => {
    switch (sev) {
      case AuditSeverity.CRITICAL:
        return 'border-red-500 text-red-500 shadow-[0_0_15px_red]';
      case AuditSeverity.WARNING:
        return 'border-yellow-500 text-yellow-500 shadow-[0_0_15px_orange]';
      case AuditSeverity.FORENSIC:
        return 'border-cyber-purple text-cyber-purple shadow-[0_0_15px_#bc13fe]';
      case AuditSeverity.SUCCESS:
        return 'border-cyber-success text-cyber-success shadow-[0_0_10px_lime]';
      default:
        return 'border-cyber-blue text-cyber-blue shadow-[0_0_10px_blue]';
    }
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-4 border-b-2 border-cyber-purple relative pb-6 shadow-[0_0_30px_rgba(188,19,254,0.2)] z-10">
        ...
        <div>
          <h2 className="text-3xl md:text-4xl font-display font-black text-white uppercase tracking-tighter mb-2 drop-shadow-lg flex items-center gap-3">
            ...
            <span
              className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 via-fuchsia-300 to-indigo-400 text-glow-purple"
              style={{ textShadow: '0 0 30px rgba(188,19,254,0.5)' }}
            >
              Bitácora Forense
            </span>
          </h2>
          <p className="text-cyber-purple/80 text-xs font-mono uppercase tracking-[0.3em] font-bold flex items-center gap-2 pl-1">
            <span className="w-2 h-2 bg-cyber-purple rounded-full animate-ping"></span> Trazabilidad
            Inmutable Activa
          </p>
        </div>
      ...
      <div className="relative group z-10">
        ...
        <div className="relative bg-[#050a14] border-2 border-cyber-purple rounded-2xl p-6 shadow-[0_0_40px_rgba(188,19,254,0.15)] flex flex-col md:flex-row gap-6 items-center overflow-hidden">
          ...
          <div className="relative flex-1 w-full group/search z-10">
            <label className="text-[9px] font-mono font-bold text-cyber-purple uppercase tracking-widest mb-2 block ml-1 text-shadow-purple">
              Rastreo de Firma Digital
            </label>
              ...
              {[
                {
                  id: 'ALL',
                  label: 'Global',
                  color: 'border-white/30 text-slate-300',
                  active:
                    'border-white bg-white/10 text-white shadow-[0_0_20px_rgba(255,255,255,0.3)]',
                },
                {
                  id: AuditSeverity.CRITICAL,
                  label: 'Crítico',
                  color: 'border-red-900 text-red-700',
                  active: 'border-red-500 bg-red-500/10 text-red-500 shadow-[0_0_20px_#ff003c]',
                },
                {
                  id: AuditSeverity.WARNING,
                  label: 'Alerta',
                  color: 'border-yellow-900 text-yellow-700',
                  active:
                    'border-yellow-500 bg-yellow-500/10 text-yellow-500 shadow-[0_0_20px_#eab308]',
                },
                {
                  id: AuditSeverity.FORENSIC,
                  label: 'Forense',
                  color: 'border-purple-900 text-purple-700',
                  active:
                    'border-cyber-purple bg-cyber-purple/10 text-cyber-purple shadow-[0_0_20px_#bc13fe]',
                },
              ].map((filter) => (
                <button
                  key={filter.id}
                  ...
                  className={`px-5 py-2.5 rounded-lg text-[10px] font-bold uppercase border-2 transition-all whitespace-nowrap hover:-translate-y-0.5 ${
                    filterSeverity === filter.id
                      ? filter.active
                      : `${filter.color} hover:border-white/50 hover:text-white bg-black`
                  }`}
                >
                  {filter.label}
                </button>
          ...
          {loading && logs.length === 0 ? (
            ...
            filteredLogs.map((log) => (
              <div key={log.id} className="relative pl-12 md:pl-24 group perspective-1000">
                ...
                <div
                  className={`absolute left-[10px] md:left-[26px] top-8 w-4 h-4 rounded-full border-2 bg-black z-20 transition-all duration-300 ${
                    log.severity === AuditSeverity.CRITICAL
                      ? 'border-red-500 shadow-[0_0_20px_red] animate-pulse bg-red-900'
                      : log.severity === AuditSeverity.WARNING
                        ? 'border-yellow-500 shadow-[0_0_15px_orange] bg-yellow-900'
                        : log.severity === AuditSeverity.FORENSIC
                          ? 'border-cyber-purple shadow-[0_0_15px_purple] bg-purple-900'
                ...
                >
                  ...
                  <div className="p-5 flex flex-col md:flex-row gap-4 items-start md:items-center justify-between relative overflow-hidden">
                    ...
                    <div className="flex items-center gap-5 relative z-10">
                      <div
                        className={`w-14 h-14 rounded-xl flex items-center justify-center text-2xl border-2 transition-transform duration-300 group-hover:rotate-6 ${getIconBoxStyle(log.severity)} bg-black`}
                      >
                      ...
                      <div>
                        <div className="text-[9px] font-mono text-slate-400 uppercase tracking-widest mb-1 flex items-center gap-2 font-bold">
                          <span
                            className={`w-1.5 h-1.5 rounded-full ${log.severity === AuditSeverity.CRITICAL ? 'bg-red-500 animate-ping' : 'bg-slate-500'}`}
                          ></span>
                          {log.type}
                        </div>
                        <div
                          className={`font-bold font-display text-sm md:text-lg text-white group-hover:text-glow transition-all uppercase tracking-wide ${log.severity === AuditSeverity.CRITICAL ? 'text-red-400 drop-shadow-[0_0_5px_red]' : ''}`}
                        >
                    ...
                    <div className="hidden md:block text-right md:text-left border-l border-white/10 pl-6 relative z-10">
                      ...
                      <div className="text-[9px] opacity-70 text-cyber-blue font-bold">
                        {log.actor_role}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-6 w-full md:w-auto justify-between md:justify-end border-l border-white/10 pl-6 relative z-10">
                      ...
                      <div
                        className={`w-8 h-8 rounded-full border border-white/10 flex items-center justify-center transition-all duration-300 ${expandedId === log.id ? 'rotate-180 bg-white/10 text-white border-white/50' : 'text-slate-500 hover:text-white hover:border-white/30'}`}
                      >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                        <div className="bg-[#050a14] border border-white/10 rounded-xl p-4 shadow-inner">
                          ...
                          <div className="space-y-3">
                            ...
                            <DetailRow
                              label="Device"
                              value={log.device_fingerprint}
                              icon="fa-desktop"
                            />
                            <DetailRow
                              label="Target"
                              value={log.target_resource || 'N/A'}
                              icon="fa-crosshairs"
                            />
                          </div>
                        </div>
                        <div className="bg-[#050a14] border border-white/10 rounded-xl p-4 shadow-inner">
                          ...
                          <div className="bg-black border-2 border-white/5 rounded-lg p-4 font-mono text-[10px] text-slate-400 break-all shadow-[inset_0_0_20px_rgba(0,0,0,1)] group-hover:border-cyber-success/30 transition-colors">
                            <div className="text-slate-500 mb-2 flex items-center gap-2 uppercase font-bold text-[8px] tracking-widest">
                              HASH (SHA-256)
                            </div>
...
const DetailRow = ({ label, value, copy, icon }: any) => (
  <div className="flex justify-between items-center border-b border-white/5 pb-2 last:border-0 group/row hover:bg-white/5 px-2 rounded transition-colors">
    ...
    <span className="text-xs font-mono text-white flex items-center gap-3">
      ...
      {copy && (
        <i
          className="fas fa-copy text-slate-600 hover:text-cyber-neon cursor-pointer transition-colors opacity-0 group-hover/row:opacity-100"
          title="Copiar"
        ></i>
      )}
    </span>
