...
export default function RiskLimitManager() {
  ...
  const theme = useMemo(() => {
    if (displayDraw.includes('Mediodía'))
      return {
        hex: '#ff5f00',
        name: 'solar',
  ...
  return (
    <div className="w-full relative group font-sans pt-6">
      ...
      <div
        className={`absolute -inset-6 opacity-40 blur-[80px] rounded-[3rem] bg-gradient-to-br ${theme.gradient} animate-[pulse_6s_ease-in-out_infinite] pointer-events-none transition-all duration-1000`}
      ></div>
      <div
        className={`absolute -inset-[2px] rounded-[2.2rem] opacity-90 blur-[2px] transition-all duration-700 animate-pulse ${theme.bg} pointer-events-none`}
      ></div>
      <div
        className={`absolute -inset-[4px] rounded-[2.3rem] opacity-40 blur-[8px] transition-all duration-700 ${theme.bg} pointer-events-none`}
      ></div>
      <div
        className={`absolute -inset-[1px] rounded-[2.1rem] border border-white/20 z-20 pointer-events-none`}
      ></div>

      {/* 3. Main Reactor Core */}
      <div
        className={`relative bg-[#02040a] border-2 ${theme.border} rounded-[2rem] overflow-hidden ${theme.shadow} flex flex-col xl:flex-row min-h-[750px] transition-all duration-500 z-10 box-border`}
      >
        {/* ====================================================================================
        ...
        <div className="xl:w-3/5 flex flex-col border-r border-white/5 relative z-10">
          ...
          <div className="p-4 md:p-6 border-b border-white/5 bg-[#05070a]/95 backdrop-blur-md relative overflow-hidden transition-colors duration-500">
            <div
              className={`absolute top-0 left-0 w-full h-[2px] ${theme.bg} shadow-[0_0_20px_currentColor] z-20`}
            ></div>
            <div
              className={`absolute top-0 left-0 w-full h-16 bg-gradient-to-b from-${theme.name === 'solar' ? 'orange' : theme.name === 'vapor' ? 'purple' : 'blue'}-500/10 to-transparent transition-all duration-700`}
            ></div>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start mb-6 relative z-10 gap-4">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <div
                    className={`w-2 h-2 rounded-full ${theme.bg} shadow-[0_0_10px_currentColor] animate-ping`}
                  ></div>
                  <span className="text-[9px] font-mono text-slate-400 uppercase tracking-[0.3em] font-bold">
                    Defensa Activa
                  </span>
                </div>
                <h2 className="text-2xl md:text-3xl font-display font-black text-white uppercase tracking-tighter flex items-center gap-3 drop-shadow-md">
                  Matriz{' '}
                  <span
                    className={`text-transparent bg-clip-text bg-gradient-to-r ${theme.gradient} filter drop-shadow-[0_0_5px_rgba(255,255,255,0.5)]`}
                  >
                    Riesgo
                  </span>
                </h2>
              ...
              <div className="text-right">
                ...
                <div
                  className={`text-2xl md:text-3xl font-mono font-black ${effectiveGlobalLimit === 0 ? 'text-red-500 drop-shadow-[0_0_10px_red]' : theme.text} drop-shadow-[0_0_15px_rgba(255,255,255,0.4)] transition-all duration-500`}
                >
                  {effectiveGlobalLimit === Infinity ? (
                    ...
                    <div className="flex items-center justify-end gap-3 relative group/neural">
                      ...
                      <div className="relative w-12 h-12 flex items-center justify-center">
                        {/* Nucleus */}
                        <div
                          className={`absolute z-20 text-xl font-bold ${theme.text} drop-shadow-[0_0_10px_currentColor]`}
                        >
                          ∞
                        </div>
                        ...
                        {/* Orbit 1 */}
                        <div
                          className={`absolute w-full h-4 border border-current rounded-[100%] animate-[spin_3s_linear_infinite] ${theme.text} opacity-60`}
                        ></div>

                        {/* Orbit 2 */}
                        <div
                          className={`absolute w-full h-4 border border-current rounded-[100%] animate-[spin_4s_linear_infinite_reverse] ${theme.text} opacity-60 rotate-60`}
                        ></div>

                        {/* Orbit 3 */}
                        <div
                          className={`absolute w-full h-4 border border-current rounded-[100%] animate-[spin_5s_linear_infinite] ${theme.text} opacity-60 -rotate-60`}
                        ></div>

                        {/* Particles */}
                        <div
                          className={`absolute w-1 h-1 bg-white rounded-full animate-[orbit_3s_linear_infinite]`}
                        ></div>
                      </div>

                      <span
                        className={`text-lg tracking-widest font-bold ${theme.text} animate-pulse drop-shadow-[0_0_5px_currentColor]`}
                      >
                        LIBRE
                      </span>
                    </div>
                ...
                <div className="text-[10px] font-mono text-slate-500 mt-1 flex items-center justify-end gap-1">
                  <span className={`w-1 h-3 ${theme.bg}`}></span> Exp:{' '}
                  ...
                  return (
                    ...
                    >
                      {isActive && (
                        <>
                          <div
                            className={`absolute inset-0 opacity-30 bg-gradient-to-t ${theme.gradient} animate-pulse`}
                          ></div>
                      ...
                      <span className="relative z-10 flex items-center gap-2">
                        <i
                          className={`fas ${d.includes('Noche') ? 'fa-moon' : 'fa-sun'} ${isActive && !isSwitching ? 'animate-bounce' : ''}`}
                        ></i>
                        {d.split(' ')[0]}
                      </span>
                    </button>
          ...
          <div className="flex-1 p-4 md:p-6 relative bg-[#020305] overflow-hidden group/grid custom-scrollbar overflow-y-auto">
            <div
              className={`absolute inset-0 bg-gradient-to-b from-transparent via-${theme.name === 'solar' ? 'orange' : theme.name === 'vapor' ? 'purple' : 'blue'}-500/10 to-transparent w-full h-[30%] pointer-events-none z-0 animate-[scan_6s_linear_infinite] opacity-30`}
            ></div>

            <div
              className={`transition-all duration-500 ease-out transform ${isSwitching ? 'scale-95 opacity-0 blur-sm' : 'scale-100 opacity-100 blur-0'}`}
            >
                ...
                {gridData.map((cell, idx) => {
                  ...
                  if (viewMode === 'SATURATION' && cell.percent > 0) {
                    bgClass = isHighRisk
                      ...
                      : isMedRisk
                        ? 'bg-yellow-900/40'
                        : `bg-${theme.name === 'solar' ? 'orange' : theme.name === 'vapor' ? 'purple' : 'blue'}-900/30`;
                    // Override borders for risk zones
                    borderClass = isHighRisk
                      ? 'border-red-500/50'
                      : isMedRisk
                        ? 'border-yellow-500/50'
                        : `${theme.border.replace('border-', 'border-opacity-50 border-')}`;
                  ...
                  return (
                    ...
                    >
                      <span
                        className={`text-[10px] sm:text-xs md:text-sm font-mono z-10 ${textClass} ${isHighRisk || isLocked ? 'animate-pulse' : ''}`}
                      >
                        {isLocked ? (
                          <i className="fas fa-lock text-[8px] sm:text-[10px] mb-1"></i>
                        ) : (
                          cell.number
                        )}
                      </span>
                      {isLocked && (
                        <span className="text-[6px] sm:text-[8px] text-red-500 font-bold">
                          BLOCK
                        </span>
                      )}
                      ...
                      {cell.percent > 0 && !isLocked && (
                        <div
                          className={`absolute inset-0 bg-white opacity-0 animate-[pulse_3s_ease-in-out_infinite]`}
                          style={{
                            animationDelay: pulseDelay,
                            animationDuration: isHighRisk ? '1s' : '3s',
                          }}
                        ></div>
                      )}
          ...
          <div className="h-8 bg-[#020202] border-t border-white/5 flex items-center overflow-hidden relative z-20">
            <div
              className={`px-4 h-full flex items-center text-[9px] font-bold ${theme.text} uppercase tracking-widest z-10 border-r border-white/10 bg-black`}
            >
            ...
            <div className="flex-1 whitespace-nowrap animate-scroll-ticker flex items-center text-[9px] font-mono text-slate-500">
              ...
              <span className="mx-6">
                LOCKED VECTORS: {gridData.filter((c) => c.limit === 0).length}
              </span>
              <span className="mx-6 text-slate-700">|</span>
          ...
          {selectedNumber ? (
            ...
            <div className="flex-1 flex flex-col p-6 md:p-8 relative z-10 animate-in slide-in-from-right-8 duration-300">
                ...
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in zoom-in duration-200">
                  <div className="border-2 border-red-500 p-8 rounded-3xl bg-red-950/80 text-center shadow-[0_0_50px_red] w-3/4">
                    ...
                    <h3 className="text-3xl font-black text-white uppercase tracking-tighter mb-2">
                      VECTOR {selectedNumber}
                    </h3>
                    <div className="text-red-500 font-display font-bold text-xl uppercase tracking-[0.5em] animate-pulse">
                      LOCKED DOWN
                    </div>
                  </div>
                </div>
              ...
              {unlockSuccess && (
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm animate-in fade-in zoom-in duration-200">
                  <div className="border-2 border-cyan-500 p-8 rounded-3xl bg-cyan-950/80 text-center shadow-[0_0_50px_cyan] w-3/4">
                    ...
                    <div className="text-cyan-400 font-display font-bold text-xl uppercase tracking-[0.5em] animate-pulse">
                      RESTORED
                    </div>
                  </div>
                </div>
              ...
              <div className="flex justify-between items-start mb-8 relative">
                ...
                <div
                  className={`absolute -inset-4 ${theme.bg} opacity-20 blur-3xl rounded-full animate-pulse`}
                ></div>
                <div>
                ...
                <div className="text-right self-center">
                  <div
                    className={`text-4xl md:text-6xl font-mono font-bold ${activeCell?.percent && activeCell.percent >= 90 ? 'text-red-500 drop-shadow-[0_0_15px_red]' : 'text-white'}`}
                  >
                    {activeCell?.percent.toFixed(0)}
                ...
                <>
                  <div className="grid grid-cols-2 gap-4 mb-8">
                    <div className="bg-white/5 border border-white/5 rounded-2xl p-4 md:p-5 hover:border-white/10 transition-colors relative overflow-hidden group/card">
                      <div className="text-[9px] text-slate-400 uppercase mb-2 font-bold tracking-wider">
                        Volumen (CRC)
                      </div>
                      <div className="text-lg md:text-2xl font-mono font-bold text-white">
                        {formatCurrency(activeCell.amount)}
                      </div>
                      <div className="w-full bg-black h-1.5 mt-3 rounded-full overflow-hidden border border-white/10">
                        <div
                          ...
                          style={{ width: `${activeCell.percent}%` }}
                        ></div>
                      </div>
                    </div>
                    <div className="bg-white/5 border border-white/5 rounded-2xl p-4 md:p-5 hover:border-white/10 transition-colors relative overflow-hidden group/card">
                      ...
                      <div
                        className={`text-lg md:text-2xl font-mono font-bold ${activeCell.limit === 0 ? 'text-red-500' : theme.text} drop-shadow-[0_0_10px_currentColor]`}
                      >
                        {activeCell.limit === Infinity ? '∞' : formatCurrency(activeCell.limit)}
                      </div>
                      <div
                        className={`text-[9px] mt-2 flex items-center gap-1 font-bold ${activeCell.isManual ? 'text-cyan-400' : 'text-slate-500'}`}
                      >
                  ...
                  <div className="mt-auto pt-6 border-t border-white/10">
                    <label className="text-[9px] text-slate-500 uppercase font-bold mb-3 block ml-1 tracking-widest">
                      Control Manual Individual
                    </label>
                    <div className="flex gap-2 mb-4">
                      <div className="relative flex-1 group/input">
                        <div
                          className={`absolute -inset-0.5 ${theme.bg} rounded-xl blur opacity-0 group-focus-within/input:opacity-60 transition-opacity duration-300`}
                        ></div>
                        <input
                          type="number"
                      ...
                      <button
                        ...
                        className={`px-6 rounded-xl font-bold uppercase transition-all shadow-lg border-2 ${
                          saveStatus === 'SUCCESS'
                            ...
                            : `bg-black ${theme.border} ${theme.text} hover:bg-white hover:text-black hover:border-white`
                        }`}
                      >
                        {saveStatus === 'SAVING' ? (
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <button
                        onClick={() => handleResetSelected(true)}
                        className="py-4 bg-white/5 border border-white/10 text-slate-400 hover:bg-white/10 hover:text-white rounded-xl text-[10px] font-bold uppercase transition-all flex items-center justify-center gap-2"
                      >
            ...
            <div className="flex-1 flex flex-col relative z-10 animate-in fade-in zoom-in duration-500 bg-[#05070a]">
              <div className="p-6 md:p-8 border-b border-white/5 bg-black/20">
                <h2 className="text-2xl font-display font-black text-white uppercase tracking-widest flex items-center gap-3">
                  <div
                    className={`w-10 h-10 rounded-full border-2 ${theme.border} flex items-center justify-center animate-spin-slow shadow-[0_0_20px_currentColor]`}
                  >
              ...
              <div className="flex-1 flex flex-col items-center justify-center relative p-8">
                ...
                <div className="relative w-[360px] h-[360px] flex items-center justify-center">
                  ...
                  <div
                    className={`absolute inset-0 opacity-10 blur-3xl rounded-full ${theme.bg}`}
                  ></div>
                  ...
                  <div className="absolute inset-0 flex items-center justify-center z-20 pointer-events-none">
                    <div
                      className={`mt-24 bg-black/80 backdrop-blur-md px-6 py-2 rounded-2xl border border-white/10 ${effectiveGlobalLimit === 0 ? 'border-red-500 shadow-[0_0_15px_red]' : `${theme.border} shadow-[0_0_15px_${theme.hex}]`}`}
                    >
                      <div
                        className={`text-4xl md:text-5xl font-mono font-black ${effectiveGlobalLimit === 0 ? 'text-red-500' : theme.text} text-shadow-sm`}
                      >
                        {effectiveGlobalLimit === Infinity ? (
                          <span className="flex items-center gap-2 tracking-widest text-2xl">
                            ILIMITADO
                          </span>
                        ) : (
                ...
                <div
                  className={`text-[10px] font-bold uppercase tracking-[0.4em] mt-8 ${effectiveGlobalLimit === Infinity ? theme.text : effectiveGlobalLimit === 0 ? 'text-red-500 animate-pulse' : theme.text} text-shadow-sm`}
                >
                  {effectiveGlobalLimit === Infinity
                    ? 'ESTADO: FLUJO LIBRE'
              ...
              <div className="p-6 md:p-8 border-t border-white/10 bg-[#020305]">
                <div className="relative bg-black border border-white/10 rounded-2xl p-1.5 flex mb-6 shadow-inner overflow-hidden h-16">
                  ...
                  >
                    {/* --- ATOM ICON SMALL --- */}
                    <i
                      className={`fas fa-atom text-xl ${effectiveGlobalLimit === Infinity ? 'animate-spin-slow' : ''}`}
                    ></i>
                    <span
                      className={
                        effectiveGlobalLimit === Infinity
                  ...
                  <button
                    onClick={() => {
                      if (effectiveGlobalLimit === Infinity) setGlobalInputValue('50000');
                    }}
                  ...
                  >
                    <i
                      className={`fas fa-lock text-xl ${effectiveGlobalLimit !== Infinity ? 'drop-shadow-[0_0_5px_red]' : ''}`}
                    ></i>
                ...
                <div
                  className={`transition-all duration-500 overflow-hidden ${effectiveGlobalLimit !== Infinity || globalInputValue ? 'max-h-40 opacity-100' : 'max-h-0 opacity-50'}`}
                >
            ...
            <div className="flex items-center justify-between mb-2">
              <div>
                ...
                <div className="text-xs text-white font-mono font-bold flex items-center gap-2">
                  AUTO-BLINDAJE
                  <span
                    className={`text-[10px] ${isBlindajeActive ? 'text-red-500 animate-pulse' : 'text-slate-600'}`}
                  >
                    ({autoShieldThreshold}%)
                  </span>
                </div>
              ...
              >
                <div
                  className={`absolute top-1 bottom-1 w-6 h-6 bg-white rounded-full transition-transform duration-300 shadow-md flex items-center justify-center ${isBlindajeActive ? 'translate-x-9' : 'translate-x-1'}`}
                >
                  {isBlindajeActive && (
                    <i className="fas fa-shield-alt text-[8px] text-red-600"></i>
                  )}
                </div>
              </button>
            </div>
            ...
            {isBlindajeActive && (
              <div className="mt-4 animate-in slide-in-from-bottom-2 fade-in">
                <div className="relative h-2 bg-slate-900 rounded-full overflow-hidden border border-white/10">
                  {/* Fill */}
                  <div
                    className="absolute top-0 left-0 h-full bg-gradient-to-r from-red-800 to-red-500 transition-all duration-100"
                    style={{ width: `${autoShieldThreshold}%` }}
                  ></div>
                </div>
                <input
                  type="range"
