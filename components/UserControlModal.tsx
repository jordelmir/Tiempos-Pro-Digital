        ...
        <div className="bg-[#050a14] border-2 border-white/10 rounded-2xl overflow-hidden relative shadow-2xl z-10 transition-colors duration-500">
          <div className="bg-black/40 border-b border-white/10 p-6 flex justify-between items-center relative overflow-hidden">
            <div className="flex items-center gap-4">
              <div
                className={`w-12 h-12 rounded bg-black border border-white/20 flex items-center justify-center relative overflow-hidden group`}
              >
                <i
                  className={`fas fa-user-shield text-2xl text-white relative z-10 ${loading ? 'animate-spin' : ''}`}
                ></i>
              </div>
              <div>
                <h3 className="font-display font-black text-white uppercase tracking-wider text-lg text-glow-sm">
                  Control de Nodo
                </h3>
          ...
          {action && (
            <div className="p-8 animate-in slide-in-from-bottom-4 duration-300">
              <button
                ...
                className="text-[10px] text-slate-500 hover:text-white mb-6 flex items-center gap-2 uppercase font-bold tracking-widest transition-colors"
              >
                <i className="fas fa-arrow-left"></i> Abortar Secuencia
              </button>
              <div
                className={`border-l-4 pl-4 mb-8 ${theme.border} transition-colors duration-500`}
              >
                <h4
                  className={`font-display font-bold text-xl uppercase ${theme.color} text-shadow`}
                >
              ...
              <div className="relative group/input mb-8">
                <input
                  ...
                  className={`relative w-full bg-black border-2 rounded-lg py-4 text-center font-mono text-xl text-white uppercase tracking-widest focus:outline-none transition-all duration-300 z-10 ${isMatch ? `${theme.border} shadow-[0_0_30px_${theme.glow}]` : 'border-slate-800 focus:border-white/50'}`}
              ...
              >
                <div
                  className={`relative z-10 flex items-center justify-center gap-3 ${isMatch ? 'text-black' : theme.color} group-hover/btn:text-black transition-colors`}
                >
                  {loading ? (
                ...
                <div
                  className={`absolute inset-0 border ${theme.border} rounded-lg opacity-50 group-hover/btn:opacity-100`}
                ></div>
              </button>
            </div>
          )}
