            ...
            <div className="relative bg-[#050a14] border-2 border-cyber-blue rounded-3xl overflow-hidden shadow-2xl z-10">
                    ...
                    <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 mb-6">
                        <div>
                            ...
                            <div className="flex items-center gap-4 text-[10px] font-mono uppercase tracking-wider text-slate-500">
                                <span><i className="fas fa-sync-alt mr-1 animate-spin-slow text-cyber-success"></i> SINC_LIVE</span>
                        ...
                        <div className="w-full xl:w-96 relative group/search">
                            ...
                            <div className="relative flex items-center bg-black border border-white/10 rounded-xl overflow-hidden shadow-inner">
                                ...
                                />
                                {entitySearch && <button onClick={() => setEntitySearch('')} className="pr-4 text-slate-500 hover:text-white"><i className="fas fa-times"></i></button>}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-4 border-t border-white/5 pt-6">
                        <div className="flex bg-black border border-white/10 rounded-xl p-1 shadow-inner overflow-x-auto no-scrollbar">
                            {['ALL', 'Mediodía', 'Tarde', 'Noche'].map((t) => (
                                <button key={t} onClick={() => setTimeFilter(t)} className={`px-4 py-2 rounded-lg text-[9px] font-bold uppercase transition-all duration-300 border whitespace-nowrap ${getFilterButtonStyle(timeFilter === t, 'TIME', t)}`}>
                                    {t === 'ALL' ? 'Todo' : t}
                                </button>
                            ))}
                        </div>
                        ...
                        <div className="flex bg-black border border-white/10 rounded-xl p-1 shadow-inner overflow-x-auto no-scrollbar">
                            {['ALL', 'WON', 'PENDING', 'LOST'].map((s) => (
                                <button key={s} onClick={() => setStatusFilter(s as any)} className={`px-4 py-2 rounded-lg text-[9px] font-bold uppercase transition-all duration-300 border whitespace-nowrap ${getFilterButtonStyle(statusFilter === s, 'STATUS', s)}`}>
                                    {s === 'ALL' ? 'Todo' : s === 'WON' ? 'Ganes' : s === 'PENDING' ? 'En Juego' : 'Cerradas'}
                                </button>
                            ))}
                        </div>
                        ...
                        {user.role !== UserRole.Cliente && (
                            <div className="flex bg-black border border-white/10 rounded-xl p-1 shadow-inner">
                                {['ALL', 'Jugador', 'Vendedor'].map((o) => (
                                    <button key={o} onClick={() => setOriginFilter(o as any)} className={`px-4 py-2 rounded-lg text-[9px] font-bold uppercase transition-all duration-300 border ${getFilterButtonStyle(originFilter === o, 'ORIGIN', o)}`}>
                                        {o === 'ALL' ? 'Origen: Todo' : o}
                                    </button>
                                ))}
                            </div>
                                    ...
                                    return (
                                        <tr key={bet.id} className="border-b border-white/5 hover:bg-white/5 transition-colors group">
                                            <td className="p-4 pl-6">
                                                ...
                                                <div className="text-[9px] text-slate-600 font-mono tracking-tighter uppercase">{bet.ticket_code}</div>
                                            </td>
                                            <td className="p-4">
                                                <div className="flex items-center gap-3">
                                                    <div className={`w-1 h-8 rounded-full ${isPlayer ? 'bg-cyber-neon' : 'bg-cyber-purple'}`}></div>
                                                    <div className="max-w-[150px] truncate">
                                                        <div className="text-white font-bold text-xs truncate">{bet.user_name}</div>
                                                        <div className={`text-[8px] uppercase tracking-wider ${isPlayer ? 'text-cyber-neon' : 'text-cyber-purple'}`}>{bet.origin}</div>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="p-4 text-center">
                                                <span className={`text-xl font-black ${isWin ? 'text-cyber-success text-glow-green animate-pulse' : 'text-white'}`}>
                                                    {bet.numbers}
                                                </span>
                                            </td>
                                            <td className="p-4 text-center">
                ...
                <div className="p-4 md:p-6 bg-[#02040a] border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-4">
                    <div className="text-[10px] font-mono text-slate-500 uppercase tracking-widest order-2 md:order-1">
                        ...
                        Mostrando <span className="text-white font-bold">{paginatedBets.length}</span> de {processedBets.length} Nodos
                    </div>
                    ...
                    <div className="flex gap-2 order-1 md:order-2">
                        ...
                        >
                            <i className="fas fa-chevron-left md:mr-2"></i> <span className="hidden md:inline">Anterior</span>
                        </button>
                        ...
                        <button 
                            ...
                            className="w-10 h-10 md:w-auto md:px-4 bg-black border border-white/10 rounded-lg text-[10px] font-bold text-slate-400 hover:text-white hover:border-white/30 disabled:opacity-30 disabled:pointer-events-none transition-all flex items-center justify-center"
                        >
                            <span className="hidden md:inline">Siguiente</span> <i className="fas fa-chevron-right md:ml-2"></i>
                        </button>
                    </div>
                    ...
                    <div className="hidden xl:flex items-center gap-3 order-3">
                        ...
                        >
                            {[10, 25, 50, 100].map(v => <option key={v} value={v}>{v} Filas</option>)}
                        </select>
                    </div>
                </div>
            </div>
    ...
    return (
        <th className={`${className} cursor-pointer hover:bg-white/5 transition-colors group/header`} onClick={() => onSort(field)}>
            <div className="flex items-center gap-2">
                {label}
                <div className={`flex flex-col text-[8px] transition-opacity duration-300 ${isActive ? 'opacity-100' : 'opacity-20 group-hover/header:opacity-100'}`}>
