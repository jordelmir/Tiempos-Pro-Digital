...
const MetricRow = ({ title, data, theme }: { title: string, data: { in: number, out: number, net: number }, theme: string }) => (
    <motion.div initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <MetricCard label="Inyecciones (+)" val={data.in} border="border-cyber-emerald" shadow="shadow-[0_0_15px_rgba(16,185,129,0.3)]" icon="fa-arrow-trend-up" color="text-cyber-emerald" />
            <MetricCard label="Retiros (-)" val={data.out} border="border-red-500" shadow="shadow-[0_0_15px_rgba(255,0,60,0.3)]" icon="fa-arrow-trend-down" color="text-red-500" />
            <MetricCard label="Delta Neto" val={data.net} border={data.net >= 0 ? "border-cyber-success" : "border-red-600"} shadow={data.net >= 0 ? "shadow-neon-green" : "shadow-neon-red"} icon={data.net >= 0 ? "fa-chart-line" : "fa-triangle-exclamation"} color={data.net >= 0 ? "text-cyber-success" : "text-red-500"} />
        </div>
    </motion.div>
);

const MetricCard = ({ label, val, border, shadow, icon, color }: any) => (
    <div className={`bg-[#050a14] border-2 ${border} ${shadow} rounded-2xl p-5 group transition-all hover:scale-[1.02] relative overflow-hidden`}>
        <div className="flex justify-between items-start mb-2 relative z-10">
            <span className="text-[10px] font-mono text-slate-500 uppercase font-black">{label}</span>
            <i className={`fas ${icon} ${color} opacity-40 group-hover:opacity-100 transition-opacity`}></i>
        </div>
        <div className={`text-2xl font-mono font-black text-white relative z-10 ${color === 'text-red-500' && val !== 0 ? 'text-red-400' : ''}`}>
            {formatCurrency(val)}
        </div>
