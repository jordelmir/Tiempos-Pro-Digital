...
const processChartData = (txs: LedgerTransaction[]) => {
    ...
    sorted.forEach(tx => {
        ...
        const dateKey = d.toLocaleDateString('es-ES', { month: 'short', year: '2-digit' }).toUpperCase();
    ...
    return Array.from(periodMap.entries()).map(([mes, val]) => ({
        mes, ...val, net: val.in - val.out
...
const aggregateLedgerByTime = (txs: LedgerTransaction[]) => {
    ...
    txs.forEach(tx => {
        ...
        if (t >= startOfYearTime) {
            if (amount > 0) stats.anual.in += amount; else stats.anual.out += Math.abs(amount);
            stats.anual.net += amount;
            if (t >= startOfMonthTime) {
                if (amount > 0) stats.mensual.in += amount; else stats.mensual.out += Math.abs(amount);
                stats.mensual.net += amount;
                if (t >= startOfWeek.getTime()) {
                    if (amount > 0) stats.semanal.in += amount; else stats.semanal.out += Math.abs(amount);
                    stats.semanal.net += amount;
                    if (t >= today) {
                        if (amount > 0) stats.diario.in += amount; else stats.diario.out += Math.abs(amount);
...
const MetricRow = ({ title, data, theme }: { title: string, data: { in: number, out: number, net: number }, theme: string }) => (
    <motion.div
        initial={{ opacity: 0, y: 10 }}
        whileInView={{ opacity: 1, y: 0 }}
        className={`space-y-6 p-6 md:p-8 rounded-[2.5rem] border-2 bg-black/40 backdrop-blur-xl transition-all duration-700 hover:scale-[1.01] ${theme === 'emerald'
...
const MetricCard = ({ label, val, border, shadow, icon, color }: any) => (
    <div className={`bg-black/60 backdrop-blur-md border-2 ${border} ${shadow} rounded-[2rem] p-6 group transition-all hover:scale-[1.05] relative overflow-hidden`}>
        <div className="flex justify-between items-start mb-3 relative z-10">
            <span className="text-[10px] font-mono text-slate-500 uppercase font-black tracking-widest">{label}</span>
            <div className={`w-8 h-8 rounded-lg bg-black/40 border border-white/10 flex items-center justify-center transition-all group-hover:border-current`}>
                <i className={`fas ${icon} ${color} opacity-60 group-hover:opacity-100 transition-opacity`}></i>
            </div>
        </div>
        <div className={`text-2xl md:text-3xl font-mono font-black text-white relative z-10 drop-shadow-[0_0_10px_rgba(255,255,255,0.3)]`}>
            {formatCurrency(val)}
        </div>
        <div className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent pointer-events-none"></div>
        {/* Dynamic scanline for the card */}
        <div className={`absolute top-0 left-0 w-full h-[1px] ${color.replace('text-', 'bg-')} opacity-20 animate-[scanline_4s_linear_infinite]`}></div>
    </div>
);
...
export default function LedgerView() {
    ...
    useEffect(() => {
        async function fetchLedger() {
            const { data } = await supabase.from('ledger_transactions').select('*').order('created_at', { ascending: false }).limit(1000);
    ...
    const H = 400, W = 1200, P = 60;
    const maxVal = Math.max(...chartData.map(d => Math.max(d.in, d.out, Math.abs(d.balance))), 1000000) * 1.2;
        ...
        <div className="p-4 md:p-8 space-y-16 animate-in fade-in duration-700">
            ...
            <header className="flex flex-col lg:flex-row justify-between items-start lg:items-end border-b-2 border-white/10 pb-10 gap-8">
                ...
                <div className="flex flex-col items-end gap-4">
                    ...
                    >
                        <i className="fas fa-file-invoice-dollar group-hover:animate-bounce"></i> Descargar Reporte Contable (CSV)
                    </button>
                </div>
            </header>
                ...
                <div className="relative bg-[#050a14]/80 border-2 border-cyber-neon shadow-neon-cyan rounded-[3.5rem] p-10 backdrop-blur-3xl overflow-hidden min-h-[600px] flex flex-col">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start mb-12 gap-8">
                        <div>
                            ...
                            <p className="text-[10px] font-mono text-slate-500 uppercase tracking-widest mt-1">Rendimiento Multidimensional (Ciclo de 12 Meses)</p>
                        </div>
                            ...
                            {activeData ? (
                                ...
                                >
                                    <div className="text-center">
                                        <div className="text-[8px] text-slate-500 font-bold uppercase mb-1">Periodo</div>
                                        <div className="text-xs font-black text-white">{activeData.mes}</div>
                                    </div>
                                    <div className="text-center">
                                        <div className="text-[8px] text-cyber-emerald font-bold uppercase mb-1">Inyecciones</div>
                                        <div className="text-xs font-black text-white">{formatCurrency(activeData.in)}</div>
                                    </div>
                                    <div className="text-center">
                                        <div className="text-[8px] text-red-500 font-bold uppercase mb-1">Retiros</div>
                                        <div className="text-xs font-black text-white">{formatCurrency(activeData.out)}</div>
                                    </div>
                                </motion.div>
                            ) : (
                    ...
                    <div className="flex-1 relative cursor-crosshair">
                        <svg viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" className="w-full h-full overflow-visible">
                            <defs>
                                <linearGradient id="barIn" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#10b981" /><stop offset="100%" stopColor="#10b981" stopOpacity="0.2" /></linearGradient>
                                <linearGradient id="barOut" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#ff003c" /><stop offset="100%" stopColor="#ff003c" stopOpacity="0.2" /></linearGradient>
                                <filter id="neon"><feGaussianBlur stdDeviation="3" result="blur" /><feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge></filter>
                            </defs>

                            {/* X-AXIS LABELS & VERTICAL LINES */}
                            {chartData.map((d, i) => (
                                <g key={`axis-${i}`}>
                                    <line x1={getX(i)} x2={getX(i)} y1={P} y2={H - P} stroke="white" strokeOpacity="0.05" />
                                    <text x={getX(i)} y={H - 10} textAnchor="middle" fill={activeIdx === i ? "#00f0ff" : "#475569"} className="text-[14px] font-mono font-bold transition-colors">{d.mes}</text>
                                </g>
                            ))}
                            ...
                            {chartData.map((d, i) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <rect x={x - 40} y={P} width="80" height={H - P * 2} fill={activeIdx === i ? "white" : "transparent"} opacity="0.03" rx="10" />

                                        {/* In Bar */}
                                        <motion.rect
                                            initial={{ height: 0 }} animate={{ height: (H - P) - getY(d.in) }}
                                            x={x - 25} y={getY(d.in)} width="20" fill="url(#barIn)" rx="4"
                                            className={`transition-opacity ${activeIdx !== null && activeIdx !== i ? 'opacity-20' : 'opacity-80'}`}
                                        />

                                        {/* Out Bar */}
                                        <motion.rect
                                            initial={{ height: 0 }} animate={{ height: (H - P) - getY(d.out) }}
                                            x={x + 5} y={getY(d.out)} width="20" fill="url(#barOut)" rx="4"
                                            className={`transition-opacity ${activeIdx !== null && activeIdx !== i ? 'opacity-20' : 'opacity-80'}`}
                                        />
                                    </g>
                                );
                            ...
                            <motion.path
                                ...
                                d={chartData.map((d, i) => `${i === 0 ? 'M' : 'L'} ${getX(i)} ${getY(d.balance)}`).join(' ')}
                                fill="none" stroke="#00f0ff" strokeWidth="4" filter="url(#neon)"
                            ...
                            {chartData.map((d, i) => (
                                <circle
                                    key={`dot-${i}`} cx={getX(i)} cy={getY(d.balance)} r={activeIdx === i ? 8 : 4}
                    ...
                    <div className="overflow-x-auto max-h-[500px] custom-scrollbar bg-black/20">
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody className="font-mono text-xs">
                                {loading ? (
                                    <tr><td colSpan={5} className="p-40 text-center animate-pulse text-cyber-neon uppercase font-black tracking-widest">Sincronizando con el Núcleo...</td></tr>
                                ) : txs.map(tx => (
                                    <tr key={tx.id} className="border-b border-white/5 hover:bg-cyber-blue/5 transition-all group/row">
                                        <td className="p-6 pl-10">
                                            <div className="font-black text-white group-hover/row:text-cyber-neon transition-colors duration-300">{tx.ticket_code || tx.id.substring(0, 8)}</div>
                                            <div className="text-[8px] text-slate-600 uppercase mt-0.5 tracking-tighter">{tx.id}</div>
                                        </td>
                                        <td className="p-6 text-slate-400 font-bold group-hover/row:text-slate-200 transition-colors">{formatDate(tx.created_at)}</td>
                                        <td className="p-6 text-center">
                                            <span className={`px-4 py-1.5 rounded-lg text-[9px] font-black border uppercase transition-all duration-300 ${tx.amount_bigint > 0 ? 'bg-cyber-emerald/10 border-cyber-emerald text-cyber-emerald shadow-neon-emerald' : 'bg-red-900/10 border-red-500 text-red-500 shadow-neon-red'}`}>
                                                {tx.type}
                                            </span>
                                        </td>
                                        <td className={`p-6 text-right font-black text-sm group-hover/row:scale-105 transition-transform duration-300 ${tx.amount_bigint > 0 ? 'text-cyber-success' : 'text-red-500'}`}>
                                            {tx.amount_bigint > 0 ? '+' : ''}{formatCurrency(tx.amount_bigint)}
                                        </td>
                                        <td className="p-6 text-right pr-10 font-black text-white text-sm group-hover/row:text-glow transition-all duration-300">{formatCurrency(tx.balance_after)}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
...
const LegendItem = ({ dot, label }: any) => (
    <div className="flex items-center gap-3">
        ...
        <span className="text-[10px] font-mono font-black text-slate-400 uppercase tracking-widest">{label}</span>
    </div>
);
