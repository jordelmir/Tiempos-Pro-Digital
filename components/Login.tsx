...
export default function Login() {
  ...
  const [viewState, setViewState] = useState<
    'IDLE' | 'BOOTING' | 'ACCESS_GRANTED' | 'ACCESS_DENIED'
  >('IDLE');
    ...
    switch (activeRole) {
      case 'VENDOR':
        ...
        return {
          ...
          inputFocus:
            'focus:border-violet-500 focus:text-violet-200 focus:shadow-[0_0_40px_rgba(139,92,246,0.5)]',
      ...
      case 'PLAYER':
        ...
        return {
          ...
          inputFocus:
            'focus:border-cyan-400 focus:text-cyan-100 focus:shadow-[0_0_40px_rgba(34,211,238,0.5)]',
      ...
      default:
        ...
        return {
          ...
          inputFocus:
            'focus:border-rose-500 focus:text-rose-200 focus:shadow-[0_0_40px_rgba(244,63,94,0.5)]',
  ...
  const renderAdminBoot = () => (
    <div className="relative h-full w-full flex items-center justify-center p-4">
      ...
      <div className="relative flex flex-col items-center justify-center h-full w-full overflow-hidden bg-black border-[6px] border-rose-600 rounded-[2.5rem] shadow-[0_0_150px_rgba(225,29,72,0.8),inset_0_0_50px_rgba(225,29,72,0.5)] group ring-4 ring-rose-900 ring-offset-8 ring-offset-black z-10">
        ...
        {/* LASER SCANNER BEAM */}
        <div
          className={`absolute left-0 w-full h-4 bg-rose-500 shadow-[0_0_50px_#f43f5e] z-20 animate-[scan_1.5s_ease-in-out_infinite] ${viewState === 'ACCESS_GRANTED' ? 'hidden' : 'block'}`}
        ></div>
        ...
        <div className="relative z-10 w-80 h-80 flex items-center justify-center perspective-1000">
          ...
          <div
            className={`absolute inset-0 rounded-full border-[6px] border-rose-900/50 border-t-rose-500 animate-[spin_4s_linear_infinite] transition-all duration-500 ${viewState === 'ACCESS_GRANTED' ? 'scale-150 opacity-0' : ''} shadow-[0_0_40px_rgba(244,63,94,0.6)]`}
          ></div>
          <div
            className={`absolute inset-6 rounded-full border-[6px] border-rose-900/50 border-b-rose-500 animate-[spin_6s_linear_infinite_reverse] transition-all duration-500 ${viewState === 'ACCESS_GRANTED' ? 'scale-125 opacity-0' : ''}`}
          ></div>
          <div
            className={`absolute inset-12 rounded-full border-4 border-dashed border-rose-800 animate-[spin_10s_linear_infinite] opacity-50`}
          ></div>

          {/* The Core Mass */}
          <div
            className={`relative w-40 h-40 bg-black rounded-full flex items-center justify-center border-4 border-rose-600 shadow-[0_0_100px_rgba(225,29,72,0.8)] overflow-hidden transition-all duration-700 ${viewState === 'ACCESS_GRANTED' ? 'scale-110 border-white shadow-[0_0_150px_white]' : ''}`}
          >
            {/* Internal Texture */}
            ...
            {/* Iris/Lens Mechanism */}
            <div
              className={`absolute inset-0 flex items-center justify-center transition-all duration-500 ${viewState === 'ACCESS_GRANTED' ? 'opacity-0' : 'opacity-100'}`}
            >
        ...
        <div className="absolute inset-0 p-8 pointer-events-none flex flex-col justify-between">
          <div className="flex justify-between items-start opacity-90">
            <div className="text-[10px] font-mono text-rose-300 font-bold border border-rose-600 px-3 py-1 bg-black/90 shadow-[0_0_20px_rgba(244,63,94,0.4)]">
              SYS.ROOT
            </div>
            <div className="text-[10px] font-mono text-rose-300 font-bold border border-rose-600 px-3 py-1 bg-black/90 shadow-[0_0_20px_rgba(244,63,94,0.4)]">
              SEC.LVL.5
          ...
          <div className="text-center pb-8 w-full flex justify-center">
            ...
            <div
              className={`font-display font-black text-lg sm:text-2xl md:text-3xl tracking-[0.2em] md:tracking-[0.3em] transition-all duration-300 whitespace-nowrap ${viewState === 'ACCESS_GRANTED' ? 'text-white drop-shadow-[0_0_30px_white]' : 'text-rose-500 animate-pulse text-shadow-neon-red'}`}
            >
    ...
    <div className="relative h-full w-full flex items-center justify-center p-4">
      ...
      <div className="relative flex flex-col items-center justify-center h-full w-full overflow-hidden bg-[#0a0514] border-[6px] border-violet-600 rounded-[2.5rem] shadow-[0_0_150px_rgba(139,92,246,0.8),inset_0_0_50px_rgba(124,58,237,0.5)] ring-4 ring-violet-900 ring-offset-8 ring-offset-black z-10">
        ...
        <div className="relative z-10 w-72 h-72 flex items-center justify-center">
          ...
          <div
            className={`absolute inset-0 border-[10px] border-violet-950 border-t-violet-500 rounded-full animate-[spin_6s_linear_infinite] shadow-[0_0_50px_rgba(139,92,246,0.7)]`}
          ></div>
          <div
            className={`absolute inset-4 border-[6px] border-indigo-900 border-b-fuchsia-500 rounded-full animate-[spin_8s_linear_infinite_reverse] shadow-[0_0_30px_rgba(217,70,239,0.6)]`}
          ></div>

          {/* Rotating Glyphs */}
          <div className="absolute inset-0 animate-[spin_20s_linear_infinite]">
            <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-6 text-violet-300 text-xl font-mono font-black drop-shadow-[0_0_15px_violet]">
              $
            </div>
            <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-6 text-violet-300 text-xl font-mono font-black drop-shadow-[0_0_15px_violet]">
              $
            </div>
          </div>
          ...
          <div
            className={`relative w-40 h-40 transform transition-all duration-700 ${viewState === 'ACCESS_GRANTED' ? 'scale-110' : 'animate-float'}`}
          >
        ...
        <div className="absolute bottom-16 text-center w-full">
          <div
            className={`font-display font-black text-xl md:text-2xl tracking-[0.2em] md:tracking-[0.3em] uppercase ${viewState === 'ACCESS_GRANTED' ? 'text-white text-shadow-white' : 'text-violet-300 text-shadow-neon-purple'}`}
          >
            {viewState === 'ACCESS_GRANTED' ? 'ASSETS VERIFIED' : 'SYNCING LEDGER'}
          </div>
          <div className="text-[10px] font-mono text-fuchsia-300 mt-2 font-bold tracking-widest">
      ...
      <div className="relative flex flex-col items-center justify-center h-full w-full overflow-hidden bg-[#02040a] border-[6px] border-cyan-400 rounded-[2.5rem] shadow-[0_0_150px_rgba(34,211,238,0.8),inset_0_0_50px_rgba(34,211,238,0.5)] ring-4 ring-cyan-800 ring-offset-8 ring-offset-black z-10">
        ...
        <div className="absolute bottom-20 w-full px-16">
          ...
          <div className="w-full h-3 bg-blue-950 rounded-full overflow-hidden border border-blue-900">
            <div
              className={`h-full bg-cyan-400 shadow-[0_0_20px_#22d3ee] ${viewState === 'ACCESS_GRANTED' ? 'w-full' : 'w-[90%] animate-pulse'}`}
            ></div>
          </div>
        </div>
      ...
      <div className="relative z-20 w-full max-w-lg perspective-1000 p-4 min-h-[600px] flex items-center justify-center">
        ...
        {viewState === 'IDLE' && (
          <div className="animate-in zoom-in-95 fade-in duration-500 relative w-full">
            ...
            <div
              className={`absolute -inset-10 ${theme.backlight} rounded-[3rem] opacity-40 blur-3xl animate-pulse transition-colors duration-700`}
            ></div>

            <div
              className={`relative bg-[#050a14]/90 backdrop-blur-2xl border-[4px] rounded-3xl p-1 group overflow-hidden z-10 transition-colors duration-700 ${theme.containerBorder} ${theme.shadow}`}
            >
              <div className="relative bg-[#020202]/95 rounded-[1.4rem] p-8 md:p-10 overflow-hidden transition-colors duration-700">
                {/* Decorative HUD Elements */}
                <div
                  className={`absolute top-4 left-4 w-3 h-3 rounded-full animate-pulse shadow-[0_0_15px_currentColor] ${theme.backlight}`}
                ></div>
                <div
                  className={`absolute top-4 right-4 text-[10px] font-black tracking-[0.3em] opacity-90 drop-shadow-[0_0_10px_currentColor] transition-colors duration-500 ${theme.primary}`}
                >
                  {theme.label}
                </div>
                ...
                <div className="text-center mb-8">
                  <div className="inline-flex items-center justify-center w-28 h-28 mb-6 relative group/icon cursor-pointer">
                    <div
                      className={`absolute inset-0 rounded-full blur-2xl opacity-40 group-hover/icon:opacity-60 animate-pulse transition-all duration-700 ${theme.backlight}`}
                    ></div>
                    <div
                      className={`relative z-10 w-full h-full rounded-full bg-black/60 border-4 group-hover/icon:border-opacity-100 transition-all duration-700 flex items-center justify-center overflow-hidden border-opacity-50 ${theme.border} ${theme.shadow}`}
                    >
                      <div
                        className={`absolute inset-0 rounded-full border-t-4 animate-[spin_3s_linear_infinite] opacity-70 ${theme.ringColor}`}
                      ></div>
                      <div
                        className={`absolute inset-2 rounded-full border-b-4 animate-[spin_2s_linear_infinite_reverse] opacity-70 ${theme.border}`}
                      ></div>
                      <i
                        className={`fas fa-fingerprint text-6xl drop-shadow-[0_0_25px_currentColor] group-hover/icon:scale-110 transition-all duration-700 ${theme.primary}`}
                      ></i>
                    </div>
                  </div>
                  ...
                  <h1 className="text-5xl font-display font-black italic tracking-tighter text-white mb-2 drop-shadow-xl">
                    TIEMPOS
                    <span
                      className={`drop-shadow-[0_0_20px_currentColor] ${theme.primary} transition-colors duration-700`}
                    >
                      PRO
                    </span>
                  </h1>
                  <p
                    className={`text-xs uppercase tracking-[0.5em] font-bold animate-pulse transition-colors duration-700 ${theme.secondaryText}`}
                  >
                    Acceso Blindado
                  </p>
                </div>
                ...
                <form onSubmit={handleLogin} className="space-y-6">
                  ...
                  <div className="space-y-2">
                    <label
                      className={`text-[10px] font-black uppercase tracking-widest ml-1 drop-shadow-[0_0_10px_currentColor] flex items-center gap-2 transition-colors duration-500 ${theme.primary}`}
                    >
                      <i className="fas fa-id-badge"></i> Identificador
                    </label>
                    <div className="relative group/input">
                      <div
                        className={`absolute -inset-1 rounded-xl blur opacity-0 group-hover/input:opacity-30 group-focus-within/input:opacity-60 transition-all duration-500 animate-pulse ${theme.backlight}`}
                      ></div>

                      <div className="relative flex items-center">
                        <div
                          className={`absolute left-4 transition-colors duration-500 ${theme.primary} opacity-90`}
                        >
                  ...
                  <div className="space-y-2">
                    <label
                      className={`text-[10px] font-black uppercase tracking-widest ml-1 drop-shadow-[0_0_10px_currentColor] flex items-center gap-2 transition-colors duration-500 ${theme.secondaryText}`}
                    >
                    ...
                    <div className="relative group/input">
                      ...
                      <div className="relative flex items-center">
                        <div
                          className={`absolute left-4 transition-colors duration-500 ${theme.secondaryText} opacity-90`}
                        >
                  ...
                  <div className="relative group/submit mt-8 perspective-500">
                    ...
                    <div
                      className={`absolute -inset-2 rounded-2xl opacity-40 blur-xl transition-all duration-500 group-hover/submit:opacity-100 group-hover/submit:blur-2xl group-hover/submit:scale-105 animate-pulse ${theme.backlight}`}
                    ></div>
                    ...
                    >
                      ...
                      <div
                        className={`absolute inset-0 opacity-30 ${theme.btnBg} mix-blend-screen`}
                      ></div>
                      ...
                      <div className="relative z-10 flex items-center justify-center gap-4">
                        <div
                          className={`p-2 rounded-lg border bg-black/50 ${theme.border} ${theme.primary} group-hover/submit:animate-spin-slow`}
                        >
                          <i className="fas fa-power-off text-xl drop-shadow-[0_0_10px_currentColor]"></i>
                        </div>
                        <span
                          className={`text-xl ${theme.btnText} drop-shadow-[0_0_15px_rgba(255,255,255,0.5)] group-hover/submit:text-white transition-colors`}
                        >
                          INICIAR SESIÓN
                        </span>
                        <i
                          className={`fas fa-chevron-right opacity-0 group-hover/submit:opacity-100 group-hover/submit:translate-x-2 transition-all duration-300 ${theme.primary}`}
                        ></i>
                      </div>
                ...
                {/* IDENTITY SELECTOR (Visual Theme Switcher) */}
                <div className="mt-8 pt-6 border-t border-white/5">
                  <div className="text-[9px] font-mono text-slate-500 uppercase tracking-[0.2em] mb-4 text-center font-bold">
                    Seleccione su Perfil de Operación
                  </div>
                  <div className="grid grid-cols-3 gap-3">
                    <button
                      type="button"
                      onClick={() => setActiveRole('ADMIN')}
                      className={`py-3 rounded-lg border transition-all duration-300 flex flex-col items-center justify-center gap-2 group relative overflow-hidden ${
                        activeRole === 'ADMIN'
                          ? 'bg-rose-950/40 border-rose-500/50 text-rose-400 shadow-[0_0_20px_rgba(225,29,72,0.3)] ring-1 ring-rose-500/50'
                          : 'bg-black/40 border-white/5 text-slate-600 hover:border-rose-500/30 hover:text-rose-400 hover:bg-rose-950/20'
                      }`}
                    >
                      <i
                        className={`fas fa-shield-alt text-lg ${activeRole === 'ADMIN' ? 'text-rose-500' : 'opacity-50'}`}
                      ></i>
                    ...
                    <button
                      ...
                      className={`py-3 rounded-lg border transition-all duration-300 flex flex-col items-center justify-center gap-2 group relative overflow-hidden ${
                        activeRole === 'VENDOR'
                          ? 'bg-[#1e1b4b]/40 border-violet-500/50 text-violet-400 shadow-[0_0_20px_rgba(139,92,246,0.3)] ring-1 ring-violet-500/50'
                          : 'bg-black/40 border-white/5 text-slate-600 hover:border-violet-500/30 hover:text-violet-400 hover:bg-[#1e1b4b]/20'
                      }`}
                    >
                      <i
                        className={`fas fa-store text-lg ${activeRole === 'VENDOR' ? 'text-violet-500' : 'opacity-50'}`}
                      ></i>
                    ...
                    <button
                      ...
                      className={`py-3 rounded-lg border transition-all duration-300 flex flex-col items-center justify-center gap-2 group relative overflow-hidden ${
                        activeRole === 'PLAYER'
                          ? 'bg-[#083344]/40 border-cyan-400/50 text-cyan-300 shadow-[0_0_20px_rgba(34,211,238,0.3)] ring-1 ring-cyan-400/50'
                          : 'bg-black/40 border-white/5 text-slate-600 hover:border-cyan-500/30 hover:text-cyan-400 hover:bg-[#083344]/20'
                      }`}
                    >
                      <i
                        className={`fas fa-gamepad text-lg ${activeRole === 'PLAYER' ? 'text-cyan-400' : 'opacity-50'}`}
                      ></i>
                      <span className="text-[8px] font-black uppercase tracking-widest">
                        Jugador
                      </span>
