  ...
  return (
    <div className="relative min-h-screen text-white font-mono overflow-hidden flex items-center justify-center">
      <div className="relative z-20 w-full max-w-lg perspective-1000 p-4 min-h-[600px] flex items-center justify-center">
        {viewState === 'IDLE' && (
            <div className="animate-in zoom-in-95 fade-in duration-500 relative w-full group">
                
                {/* ALIVE GLOW: LOGIN CONTAINER */}
                <div className={`absolute -inset-4 ${theme.backlight}/20 rounded-[3rem] blur-3xl animate-pulse transition-all duration-1000 opacity-20`}></div>
                
                {/* GLASS MORPHISM ENHANCED CONTAINER */}
                <div className={`relative glass-morphism bg-black/30 backdrop-blur-3xl border-2 rounded-[2.5rem] p-1 group overflow-hidden z-10 transition-all duration-700 ${theme.containerBorder} ${theme.shadow} hover:border-white/20`}>
                    
                    <div className="relative bg-black/40 rounded-[2.2rem] p-8 md:p-12 overflow-hidden transition-colors duration-700">
                        {/* Dynamic Scanline Overlay */}
                        <div className={`absolute top-0 left-0 w-full h-[1px] ${theme.backlight} opacity-30 shadow-[0_0_15px_currentColor] animate-[scanline_4s_linear_infinite]`}></div>
                        
                        <div className={`absolute top-6 left-6 w-3 h-3 rounded-full animate-pulse shadow-[0_0_15px_currentColor] ${theme.backlight}`}></div>
                        <div className={`absolute top-6 right-8 text-[10px] font-black tracking-[0.3em] opacity-90 drop-shadow-[0_0_10px_currentColor] transition-colors duration-500 ${theme.primary}`}>
                            {theme.label}
                        </div>
                        
                        <div className="text-center mb-10">
                            <div className="inline-flex items-center justify-center w-28 h-28 mb-8 relative group/icon cursor-pointer">
                                ...
                                <div className={`relative z-10 w-full h-full rounded-full bg-black/60 border-4 group-hover/icon:border-opacity-100 transition-all duration-700 flex items-center justify-center overflow-hidden border-opacity-50 ${theme.border} ${theme.shadow} backdrop-blur-md`}>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="relative group/input">
                                    ...
                                    <div className="relative flex items-center">
                                        ...
                                        <input type="email" required value={email} onChange={e => setEmail(e.target.value)} className={`relative w-full bg-black/60 border-2 rounded-xl py-4 pl-12 pr-4 text-white font-mono text-sm placeholder-slate-700 focus:outline-none transition-all duration-500 z-10 backdrop-blur-sm ${theme.inputBorder} ${theme.inputFocus}`} placeholder="OPERADOR@NET.LOCAL" />
                                    </div>
                                </div>
                            </div>
                            <div className="space-y-3">
                                ...
                                <div className="relative group/input">
                                    ...
                                    <div className="relative flex items-center">
                                        ...
                                        <input type="password" required value={password} onChange={e => setPassword(e.target.value)} className={`relative w-full bg-black/60 border-2 rounded-xl py-4 pl-12 pr-4 text-white font-mono text-sm placeholder-slate-700 focus:outline-none transition-all duration-500 z-10 backdrop-blur-sm ${theme.inputBorder} ${theme.inputFocus}`} placeholder="••••••••••••" />
                                    </div>
                                </div>
                            </div>
                            <div className="relative group/submit mt-10 perspective-500">
                                ...
                                <button type="submit" className={`relative w-full py-6 rounded-xl font-display font-black uppercase tracking-[0.2em] overflow-hidden transition-all duration-300 border-2 bg-black/80 backdrop-blur-xl ${theme.border} group-hover/submit:border-white/50 group-hover/submit:scale-[1.02] shadow-2xl`}>
                                    ...
                                    <div className="relative z-10 flex items-center justify-center gap-4">
                                        ...
                                        <span className={`text-xl ${theme.btnText} drop-shadow-[0_0_15px_rgba(255,255,255,0.5)] group-hover/submit:text-white transition-colors`}>INICIAR SESIÓN</span>
                        ...
                        <div className="mt-12 pt-8 border-t border-white/10">
                            <div className="text-[10px] font-mono text-slate-500 uppercase tracking-[0.3em] mb-6 text-center font-bold">Protocolo de Acceso Rápido</div>
                            <div className="grid grid-cols-3 gap-5">
                                <button onClick={() => fillCredentials('ADMIN')} className={`py-4 rounded-xl border-[3px] transition-all duration-300 flex flex-col items-center justify-center gap-2 group relative overflow-hidden backdrop-blur-sm ${activeRole === 'ADMIN' ? 'bg-rose-950/60 border-rose-500 text-rose-400 shadow-[0_0_30px_rgba(225,29,72,0.6)] scale-105 ring-2 ring-offset-2 ring-offset-black ring-rose-500' : 'bg-black/40 border-white/10 text-slate-600 hover:border-rose-500/50 hover:text-rose-400 hover:bg-rose-900/20'}`}>
                                    <i className={`fas fa-shield-alt text-xl ${activeRole === 'ADMIN' ? 'animate-pulse' : ''}`}></i>
                                    <span className="text-[9px] font-black uppercase tracking-widest">Admin</span>
                                </button>
                                <button onClick={() => fillCredentials('VENDOR')} className={`py-4 rounded-xl border-[3px] transition-all duration-300 flex flex-col items-center justify-center gap-2 group relative overflow-hidden backdrop-blur-sm ${activeRole === 'VENDOR' ? 'bg-[#1e1b4b]/70 border-violet-500 text-violet-400 shadow-[0_0_30px_rgba(139,92,246,0.6)] scale-105 ring-2 ring-offset-2 ring-offset-black ring-violet-500' : 'bg-black/40 border-white/10 text-slate-600 hover:border-violet-500/50 hover:text-violet-400 hover:bg-[#1e1b4b]/40'}`}>
                                ...
                                <button onClick={() => fillCredentials('PLAYER')} className={`py-4 rounded-xl border-[3px] transition-all duration-300 flex flex-col items-center justify-center gap-2 group relative overflow-hidden backdrop-blur-sm ${activeRole === 'PLAYER' ? 'bg-[#083344]/70 border-cyan-400 text-cyan-300 shadow-[0_0_30px_rgba(34,211,238,0.6)] scale-105 ring-2 ring-offset-2 ring-offset-black ring-cyan-400' : 'bg-black/40 border-white/10 text-slate-600 hover:border-cyan-500/50 hover:text-cyan-400 hover:bg-[#083344]/40'}`}>
