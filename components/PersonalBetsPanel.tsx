...
export default function PersonalBetsPanel({ theme, refreshTrigger }: PersonalBetsPanelProps) {
  ...
  return (
    <div className="relative group h-[600px] flex flex-col">
      ...
      <div
        className={`absolute -inset-1 ${theme.glow} rounded-2xl opacity-10 blur-xl animate-pulse transition-all duration-1000`}
      ></div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar relative p-4 space-y-3">
          ...
          {loading ? (
            ...
            filteredBets.map((bet) => {
              ...
              if (isWin) {
                statusColor =
                  'border-cyber-success/50 bg-cyber-success/5 shadow-[0_0_20px_rgba(10,255,96,0.1)]';
                icon = 'fa-trophy';
              } else if (isPending) {
                ...
                >
                  ...
                  <div className="relative z-10 flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-4">
                      <div
                        className={`w-12 h-12 rounded-lg bg-black border border-white/10 flex items-center justify-center text-2xl font-mono font-bold text-white shadow-inner ${isWin ? 'text-cyber-success border-cyber-success shadow-[0_0_10px_lime]' : ''}`}
                      >
                        {bet.numbers}
                      </div>
                      <div>
                        <div className="text-[9px] font-mono text-slate-500 uppercase tracking-wider mb-0.5 flex items-center gap-2">
                          ...
                          {bet.ticket_code && (
                            <span className="text-[8px] bg-white/10 px-1 rounded text-slate-400">
                              {bet.ticket_code}
                            </span>
                          )}
                        </div>
                    ...
                    <div className="text-right">
                      <div
                        className={`text-sm font-mono font-bold ${isWin ? 'text-cyber-success text-glow-green' : 'text-white'}`}
                      >
                        {formatCurrency(bet.amount_bigint)}
                      </div>
                      <div
                        className={`text-[9px] font-bold uppercase tracking-widest mt-1 flex flex-col items-end gap-1 ${
                          isWin
                            ? 'text-cyber-success'
                            : isPending
                              ? 'text-blue-400'
                      ...
                      >
                        ...
                        {isWin && (
                          <span className="text-cyber-success font-mono font-black text-[11px] drop-shadow-[0_0_10px_rgba(10,255,96,1)]">
