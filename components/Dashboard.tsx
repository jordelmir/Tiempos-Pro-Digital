...
export default function Dashboard() {
    ...
    const handleExecuteBatch = async () => {
        if (isMarketClosed || pendingBets.length === 0 || !user) {
            console.warn("Batch Execution Blocked:", { isMarketClosed, bets: pendingBets.length, user: !!user });
            if (isMarketClosed) alert("MERCADO CERRADO: Esperando sincronización de reloj o fuera de horario.");
    ...
    return (
        <div className="p-4 md:p-8 space-y-12 md:space-y-24 relative overflow-x-hidden">
            ...
            {isAdmin && <AdminResultControl isOpen={adminResultOpen} onClose={() => setAdminResultOpen(false)} initialDraw={null} onPublishSuccess={() => { setTableRefreshTrigger(p => p + 1); fetchUser(true); }} />}
