...
export default function UserManagementPanel({
  ...
  const [activeTab, setActiveTab] = useState<'CLIENTES' | 'VENDEDORES'>(
    currentUser?.role === UserRole.Vendedor ? 'CLIENTES' : 'CLIENTES'
  );

  // Still keep it for dynamic role changes if they ever happen without remount
  useEffect(() => {
    if (isVendor && activeTab !== 'CLIENTES') {
  ...
  const totalBalance = useMemo(
    () => filteredUsers.reduce((acc, curr) => acc + curr.balance_bigint, 0),
    [filteredUsers]
  );
  ...
  return (
    <div className="relative group animate-in fade-in duration-500">
      ...
      <div
        className={`absolute -inset-1 ${theme.bg} rounded-[2rem] opacity-20 blur-2xl transition-opacity duration-1000 animate-pulse`}
      ></div>
      <div
        className={`absolute -inset-[1px] ${theme.bg} rounded-[2rem] opacity-40 blur-md transition-opacity duration-700`}
      ></div>

      {/* Main Container - SOLID CORE WITH PHOSPHORESCENT BORDER */}
      <div
        className={`relative bg-[#050a14] border-2 ${theme.border} rounded-3xl overflow-hidden transition-colors duration-500 z-10 shadow-2xl`}
      >
        {/* Header Area */}
        <div className="border-b border-white/5 bg-[#02040a]/80 backdrop-blur-xl relative overflow-hidden">
          {/* Top Energy Line */}
          <div
            className={`absolute top-0 left-0 w-full h-[2px] ${theme.bg} shadow-[0_0_15px_currentColor] opacity-50`}
          ></div>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between p-6 pb-0 md:pb-0 relative z-10">
            <h3 className="text-xl font-display font-black text-white uppercase tracking-widest flex items-center gap-4 mb-6 md:mb-0 drop-shadow-md">
              <div
                className={`w-12 h-12 rounded-xl bg-black border-2 ${theme.border} flex items-center justify-center ${theme.shadow} transition-colors duration-500`}
              >
                <AnimatedIconUltra
                  profile={{
                    animation: 'spin3d',
                    speed: 4,
                    size: 0.9,
                    theme: activeTab === 'CLIENTES' ? 'cyber' : 'neon',
                  }}
                >
              ...
              <span>
                Directorio{' '}
                <span className={`${theme.text} text-glow-sm transition-colors duration-500`}>
            ...
            {isAdmin && (
              <div
                className={`flex bg-black/60 p-1.5 rounded-2xl border-2 ${theme.border} shadow-lg transition-colors duration-500`}
              >
                <button
          ...
          <div className="p-6 grid grid-cols-1 lg:grid-cols-12 gap-6 items-end relative z-10">
            ...
            <div className="lg:col-span-5 relative group/input">
              <label
                className={`text-[10px] font-mono font-bold ${theme.text} uppercase tracking-widest ml-1 mb-1 block transition-colors duration-500`}
              >
              ...
              <div className="relative">
                {/* Input Glow on Focus */}
                <div
                  className={`absolute -inset-0.5 ${theme.bg} rounded-xl blur opacity-0 group-focus-within/input:opacity-50 transition-opacity duration-500`}
                ></div>

                <div
                  className={`relative flex items-center bg-black border-2 ${theme.border} rounded-xl overflow-hidden transition-colors duration-500 shadow-inner group-focus-within/input:${theme.shadow}`}
                >
                  <div className={`pl-4 ${theme.text} opacity-70`}>
            ...
            <div className="lg:col-span-4 flex items-center justify-end gap-4">
              <div className="text-right">
                ...
                <div
                  className={`text-xl font-mono font-bold ${theme.text} text-glow-sm transition-colors duration-500`}
                >
              ...
              <div className="text-right">
                ...
                <div className="text-lg font-mono font-bold text-white">
                  ...
                  <span className="text-red-500">{suspendedCount}</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="relative overflow-x-auto min-h-[auto] max-h-[600px] custom-scrollbar bg-[#080c14]">
          <table className="w-full text-left border-collapse relative z-10">
            <thead className="sticky top-0 bg-[#02040a] z-20 shadow-xl">
              <tr
                className={`text-[9px] font-mono ${theme.text} uppercase border-b border-white/10 tracking-widest transition-colors duration-500`}
              >
                  ...
                  >
                    <td className="p-4 pl-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div
                          className={`relative w-10 h-10 rounded-full bg-black flex items-center justify-center border-2 border-white/10 ${theme.hoverBorder} ${theme.hoverShadow} transition-[border-color,box-shadow] duration-300`}
                        >
                          ...
                          <div className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-black rounded-full flex items-center justify-center">
                            <div
                              className={`w-1.5 h-1.5 rounded-full ${u.status === 'Active' ? 'bg-green-500 shadow-[0_0_5px_lime]' : 'bg-red-500 shadow-[0_0_5px_red]'}`}
                            ></div>
                          </div>
                        </div>
                    ...
                    <td className="p-4">
                      <div className="flex flex-col">
                        <span
                          className={`text-[10px] font-bold ${theme.text} tracking-wider transition-colors duration-500`}
                        >
                          ID: {u.cedula || 'N/A'}
                        </span>
                        <span className="text-[9px] text-slate-500">{u.phone}</span>
                        {u.email && (
                          <span className="text-[8px] text-slate-600 opacity-70">{u.email}</span>
                        )}
                      </div>
                    </td>
                    <td className="p-4 text-right">
                      <div
                        className={`font-bold text-sm ${u.balance_bigint > 0 ? theme.text : 'text-slate-500'} text-glow-sm transition-colors duration-500`}
                      >
                        {formatCurrency(u.balance_bigint)}
                      </div>
                      {activeTab === 'VENDEDORES' && (
                        <button
                          onClick={() => setPayUser(u)}
                          className="mt-1 px-2 py-0.5 bg-cyber-purple/10 border border-cyber-purple/30 rounded text-[8px] font-bold text-cyber-purple hover:bg-cyber-purple hover:text-black transition-colors ml-auto shadow-[0_0_10px_rgba(188,19,254,0.1)]"
                        >
                          LIQUIDAR
                        </button>
                      )}
                    ...
                    <td className="p-4 text-center">
                      <div
                        className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[9px] font-bold uppercase border-2 transition-colors duration-300 ${
                          u.status === 'Active'
                      ...
                      >
                        <span
                          className={`w-1.5 h-1.5 rounded-full ${u.status === 'Active' ? 'bg-cyber-success animate-pulse' : 'bg-cyber-danger'}`}
                        ></span>
                    ...
                    <td className="p-4 pr-6 text-right">
                      <div className="flex items-center justify-end gap-3 opacity-90">
                        ...
                        >
                          <i
                            className={`fas ${u.status === 'Active' ? 'fa-lock' : 'fa-unlock'}`}
                          ></i>
                        </button>
                      </div>
                    </td>
        ...
        <div className="bg-[#02040a] p-3 border-t border-white/5 flex justify-between items-center text-[10px] font-mono text-slate-500 relative z-20">
          <div>
            MOSTRANDO {filteredUsers.length} DE {sourceList.length} REGISTROS
          </div>
          <div className="flex gap-4">
            <span className="flex items-center gap-1">
              <i className="fas fa-circle text-[6px] text-green-500 animate-pulse"></i> SYSTEM_READY
            </span>
          </div>
        </div>
      </div>
