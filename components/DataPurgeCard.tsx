...
export default function DataPurgeCard({ theme: parentTheme }: { theme?: any }) {
  ...
  // UI Local State
  const [view, setView] = useState<'SELECT' | 'ANALYZE' | 'CONFIRM' | 'EXECUTING' | 'SUCCESS'>(
    'SELECT'
  );
  ...
  const TARGETS: { id: PurgeTarget; label: string; icon: string; color: string; desc: string }[] = [
    {
      id: 'BETS_HISTORY',
      label: 'Apuestas Pasadas',
      icon: 'fa-ticket-alt',
      color: 'text-blue-400',
      desc: 'Limpia jugadas finalizadas (Ganadas/Perdidas) para liberar memoria operativa.',
    },
    {
      id: 'AUDIT_LOGS',
      ...
      desc: 'Depura registros de actividad técnica antiguos. No afecta datos de usuario.',
    },
    {
      id: 'RESULTS_HISTORY',
      label: 'Historial Sorteos',
      icon: 'fa-history',
      color: 'text-orange-400',
      desc: 'Elimina registros de resultados de días anteriores del búfer visual.',
    },
    {
      id: 'LEDGER_OLD',
      ...
      desc: 'Remueve transacciones bancarias antiguas del registro público. El saldo está blindado.',
    },
  ];
  ...
  return (
    <div className="relative w-full group font-sans">
      ...
      <div
        className={`absolute -inset-1 rounded-[2rem] opacity-30 blur-xl animate-pulse transition-all duration-1000 bg-red-600`}
      ></div>

      {/* Main Chassis - Permanent Red Phosphorescent Border */}
      <div
        className={`relative bg-[#050a14]/90 border-2 border-red-600 shadow-neon-red rounded-[2rem] overflow-hidden flex flex-col min-h-[480px] transition-all duration-700 z-10 backdrop-blur-2xl`}
      >
        {/* Header Estilo Terminal */}
        <div
          className={`px-6 py-4 border-b border-white/5 flex items-center justify-between bg-red-950/30`}
        >
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-black border border-white/10 flex items-center justify-center shadow-inner">
              <AnimatedIconUltra
                profile={{ animation: 'spin3d', speed: 4, theme: 'neon', size: 0.8 }}
              >
                <i
                  className={`fas ${view === 'CONFIRM' ? 'fa-radiation text-red-500' : 'fa-server text-red-500'} text-sm`}
                ></i>
              </AnimatedIconUltra>
            </div>
            <div>
              <h3 className="text-xs md:text-sm font-display font-black text-white uppercase tracking-[0.2em]">
                Mantenimiento Forense
              </h3>
              <p className="text-[8px] font-mono text-red-400 uppercase tracking-widest">
                Protocolo de Optimización v3.6.1
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <span
              className={`w-1.5 h-1.5 rounded-full ${view === 'EXECUTING' ? 'bg-red-500 animate-ping' : 'bg-red-500'} shadow-[0_0_5px_currentColor]`}
            ></span>
        ...
        <div className="p-6 flex-1 flex flex-col">
                ...
                {TARGETS.map((t) => (
                  ...
                  >
                    <div className="relative z-10">
                      <AnimatedIconUltra
                        profile={{ animation: 'spin3d', speed: 3.5, theme: 'cyber', size: 1.1 }}
                      >
                        <i
                          className={`fas ${t.icon} ${t.color} text-2xl mb-3 opacity-60 group-hover/btn:opacity-100 transition-opacity`}
                        ></i>
                      </AnimatedIconUltra>
                      <div className="text-[10px] font-black text-white uppercase tracking-wider">
                        {t.label}
                      </div>
                      <div className="text-[8px] text-slate-600 mt-1 uppercase font-bold leading-tight line-clamp-2">
                        {t.desc}
                      </div>
                    </div>
                ...
                <div className="flex justify-between items-center mb-4">
                  <div>
                    ...
                    <p className="text-[8px] text-slate-500 uppercase font-bold">
                      Conserva datos de los últimos {days} días
                    </p>
                  </div>
                  <div className="text-xl font-mono font-black text-white bg-red-600 px-3 py-1 rounded-lg shadow-neon-red">
                    {days}d
                  </div>
                </div>
                <input
          ...
          {view === 'ANALYZE' && (
            <div className="flex-1 flex flex-col items-center justify-center space-y-4 py-12 animate-pulse">
              ...
              <h2 className="text-[12px] font-black text-white uppercase tracking-[0.5em]">
                Escaneando Sectores de Red...
              </h2>
              <p className="text-[9px] font-mono text-red-900 animate-pulse">
                Analizando integridad de punteros...
              </p>
            </div>
          )}
            ...
            <div className="flex-1 flex flex-col animate-in slide-in-from-bottom-6 duration-500">
              <div className="bg-black/60 border-2 border-red-900/30 rounded-3xl p-6 mb-8 relative overflow-hidden">
                ...
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <h4 className="text-white font-black uppercase text-sm tracking-widest mb-1">
                      Reporte de Impacto
                    </h4>
                    <p className="text-[10px] text-red-400/80 font-mono uppercase font-bold">
                      Objetivo: {target}
                    </p>
                  </div>
                  <div
                    className={`px-3 py-1 rounded-lg text-[9px] font-black border ${analysis.riskLevel === 'HIGH' ? 'bg-red-950 text-red-500 border-red-500/50 shadow-neon-red' : 'bg-yellow-950 text-yellow-500 border-yellow-500/50 shadow-neon-orange'}`}
                  >
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  ...
                  <div className="bg-black p-4 rounded-xl border border-white/5 flex flex-col items-center">
                    ...
                    <div className="text-3xl font-mono font-bold text-red-500">
                      {analysis.estimatedSizeKB} <span className="text-sm">KB</span>
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-4">
                <label className="text-[9px] font-black text-slate-500 uppercase tracking-[0.4em] block text-center">
                  Escriba para autorizar ignición
                </label>
                <input
                  type="text"
                  value={confirmPhrase}
              ...
              <div className="mt-8 flex gap-3">
                <button
                  onClick={reset}
                  className="flex-1 py-4 bg-white/5 hover:bg-white/10 text-slate-500 hover:text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all"
                >
                  Abortar
                </button>
                <button
          ...
          {view === 'EXECUTING' && (
            <div className="flex-1 flex flex-col items-center justify-center py-12">
              ...
              <h2 className="text-xl font-display font-black text-white uppercase tracking-[0.5em] mb-4">
              ...
              <p className="text-[9px] font-mono text-red-400 mt-4 uppercase animate-pulse">
                Inyectando comandos de limpieza atómica...
              </p>
            </div>
          )}
          ...
          {view === 'SUCCESS' && (
            <div className="flex-1 flex flex-col items-center justify-center py-12 animate-in zoom-in-95 duration-500 text-center">
              ...
              <div className="mt-6 bg-red-950/20 border border-red-600/30 p-5 rounded-2xl min-w-[240px]">
                <p className="text-[10px] font-mono text-red-400 uppercase tracking-widest mb-1 font-bold">
                  Registros Liberados
                </p>
                <div className="text-4xl font-mono font-black text-white drop-shadow-[0_0_10px_rgba(255,0,60,0.5)]">
                  {purgeResult}
                </div>
              </div>
              <button
                onClick={reset}
                className="mt-10 px-12 py-4 bg-white text-black hover:bg-red-500 transition-all rounded-full text-xs font-black uppercase tracking-widest shadow-xl active:scale-95"
              >
                Regresar al Núcleo
              </button>
            </div>
        ...
        <div className="px-6 py-4 bg-black/60 border-t border-white/5 flex items-center gap-4">
          ...
          <p className="text-[9px] font-mono text-red-900 uppercase tracking-widest leading-relaxed font-bold">
            Protocolo Phront Maestro: Las apuestas ACTIVAS y balances actuales están BLINDADOS por
            el Kernel y no pueden ser purgados.
          </p>
        </div>
      </div>
