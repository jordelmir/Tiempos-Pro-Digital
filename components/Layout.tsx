...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div
      className={`min-h-screen flex flex-col text-slate-200 font-sans selection:bg-cyber-neon selection:text-black`}
    >
      ...
      {showLogoutConfirm && (
        <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="bg-[#050a14] border-2 border-cyber-danger rounded-2xl max-w-sm w-full shadow-[0_0_50px_rgba(255,0,60,0.3)] text-center relative overflow-hidden group">
            ...
            <h3 className="text-xl font-display font-bold text-white mb-2 uppercase tracking-widest text-shadow-red">
              ¿Desconectar?
            </h3>
            <p className="text-cyber-danger/70 text-xs font-mono mb-8">
              Se cerrará el túnel seguro con el Núcleo.
            </p>
            <div className="flex gap-4 p-6 border-t border-white/10 bg-black/40">
              <button
                onClick={() => setShowLogoutConfirm(false)}
                className="flex-1 py-3 rounded-lg border border-slate-700 hover:bg-slate-800 text-slate-300 font-bold uppercase text-xs tracking-wider transition-colors shadow-[0_0_10px_rgba(255,255,255,0.05)]"
              >
                Cancelar
              </button>
              <button
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between relative group-header">
          ...
          <div className="flex items-center gap-4">
            <div className="relative group/capsule hidden md:flex">
              {/* Living Backlight */}
              <div
                className={`absolute -inset-1 rounded-full opacity-20 blur-md animate-pulse group-hover/capsule:opacity-40 transition-opacity duration-500 ${
              ...
              <div className="relative flex items-center bg-[#050a14] border border-white/10 rounded-full pl-1 pr-1 py-1 gap-3 shadow-inner transition-all duration-500">
                ...
                {/* Role Badge */}
                <div
                  className={`flex items-center justify-center px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-wider border ${
                    isAdmin
                      ? 'bg-cyber-emerald/10 border-cyber-emerald/50 text-cyber-emerald'
                      : isVendor
        ...
        {isMobileMenuOpen && (
          <div className="fixed top-16 left-0 right-0 bg-[#050a14]/95 backdrop-blur-xl border-b border-cyber-neon shadow-2xl p-4 animate-in slide-in-from-top-4 duration-300 z-40">
            ...
            <div className="mt-4 pt-4 border-t border-white/10">
              ...
              <button
                onClick={requestSignOut}
                className="w-full mt-2 py-3 bg-cyber-danger/10 border border-cyber-danger text-cyber-danger rounded-xl font-bold uppercase text-xs tracking-wider flex items-center justify-center gap-2 shadow-neon-red"
              >
      ...
      <footer className="fixed bottom-0 left-0 w-full bg-[#02040a] border-t border-cyber-border/50 z-40 h-8 flex items-center overflow-hidden shadow-[0_-5px_20px_rgba(30,58,138,0.3)]">
        <div className="flex-shrink-0 px-4 bg-cyber-blue/20 h-full flex items-center border-r border-cyber-blue/30 z-10">
          ...
          <span className="text-[9px] font-mono font-bold text-cyber-blue tracking-widest">
            LIVE
          </span>
        </div>
        ...
        <div className="flex-1 overflow-hidden relative">
          <div className="absolute whitespace-nowrap animate-scroll-ticker flex gap-12 items-center text-[9px] font-mono text-slate-500 uppercase tracking-wider top-1.5">
            <span>
              <i className="fas fa-server text-cyber-purple mr-2"></i> Edge-Node-01: ONLINE
            </span>
            <span>
              <i className="fas fa-shield-alt text-cyber-success mr-2"></i> Integrity: 100%
            </span>
            <span>
              <i className="fas fa-clock text-cyber-neon mr-2"></i> Sync Latency: 12ms
            </span>
            <span>
              <i className="fas fa-database text-blue-500 mr-2"></i> Ledger Hash: Verified
            </span>
            <span>
              <i className="fas fa-users text-cyber-orange mr-2"></i> Active Users: 1,042
            </span>
            <span>
              <i className="fas fa-bolt text-yellow-400 mr-2"></i> Voltage: Stable
            </span>
            <span>
              <i className="fas fa-server text-cyber-purple mr-2"></i> Edge-Node-01: ONLINE
            </span>
...
const MobileNavItem = ({ label, icon, onClick, active, color }: any) => {
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full flex items-center gap-4 px-4 py-3 rounded-xl transition-all duration-300 border ${active ? `bg-black ${themeColors[color]}` : 'border-white/10 text-slate-400 hover:border-white/30 hover:text-white'}`}
    >
