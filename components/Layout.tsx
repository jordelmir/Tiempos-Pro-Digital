        ...
        <div className={`min-h-screen flex flex-col text-slate-200 font-sans selection:bg-cyber-neon selection:text-black`}>
            ...
            <header className="fixed top-0 left-0 right-0 z-50 w-full border-b border-white/10 bg-[#050a14]/95 backdrop-blur-xl shadow-lg">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between relative group-header">
                    ...
                    {/* 1. IDENTITY MODULE */}
                    <div className="flex items-center gap-4 cursor-pointer" onClick={() => router.push(ROUTES.DASHBOARD)}>
                    ...
                    <nav className="hidden lg:flex items-center gap-1 mx-8">
                        <NavItem icon="fa-chart-pie" label="Panel" active={isActive(ROUTES.DASHBOARD)} onClick={() => router.push(ROUTES.DASHBOARD)} color="cyber-neon" />

                        {/* ADMIN ONLY MODULES */}
                        {isAdmin && (
                            <>
                                <NavItem icon="fa-dna" label="Auditoría" active={isActive(ROUTES.AUDIT)} onClick={() => router.push(ROUTES.AUDIT)} color="cyber-purple" />
                                <NavItem icon="fa-server" label="Libro" active={isActive(ROUTES.LEDGER)} onClick={() => router.push(ROUTES.LEDGER)} color="cyber-success" />
                            </>
                        )}
                    </nav>
                    ...
                    <div className="flex items-center gap-4">
                        <div className="relative group/capsule hidden md:flex">
                            {/* Living Backlight */}
                            <div className={`absolute -inset-1 rounded-full opacity-20 blur-md animate-pulse group-hover/capsule:opacity-40 transition-opacity duration-500 ${isAdmin ? 'bg-cyber-emerald' : isVendor ? 'bg-cyber-purple' : 'bg-cyber-neon'
                                }`}></div>
                            ...
                            <div className="relative flex items-center bg-[#050a14] border border-white/10 rounded-full pl-1 pr-1 py-1 gap-3 shadow-inner transition-all duration-500">
                                ...
                                {/* Role Badge */}
                                <div className={`flex items-center justify-center px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-wider border ${isAdmin ? 'bg-cyber-emerald/10 border-cyber-emerald/50 text-cyber-emerald' :
                ...
                {isMobileMenuOpen && (
                    <div className="fixed top-16 left-0 right-0 bg-[#050a14]/95 backdrop-blur-xl border-b border-cyber-neon shadow-2xl p-4 animate-in slide-in-from-top-4 duration-300 z-40">
                        <div className="space-y-2">
                            <MobileNavItem label="Panel Principal" icon="fa-chart-pie" color="neon" onClick={() => { router.push(ROUTES.DASHBOARD); setIsMobileMenuOpen(false); }} active={isActive(ROUTES.DASHBOARD)} />

                            {/* ADMIN ONLY MOBILE MODULES */}
                            {isAdmin && (
                                <>
                                    <MobileNavItem label="Auditoría Forense" icon="fa-dna" color="purple" onClick={() => { router.push(ROUTES.AUDIT); setIsMobileMenuOpen(false); }} active={isActive(ROUTES.AUDIT)} />
                                    <MobileNavItem label="Libro Mayor" icon="fa-server" color="success" onClick={() => { router.push(ROUTES.LEDGER); setIsMobileMenuOpen(false); }} active={isActive(ROUTES.LEDGER)} />
                                </>
                            )}
                        </div>
