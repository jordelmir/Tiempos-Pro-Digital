        ...
        <div className="relative max-w-sm w-full perspective-1000 z-10 my-auto">
            ...
            >
                ...
                >
                    ...
                    <span className="text-[6px] font-black uppercase tracking-tighter opacity-40 group-hover:opacity-100">VOID</span>
                </button>
                ...
                <div className="pl-8 pr-6 py-6 md:py-8 relative overflow-y-auto custom-scrollbar">
                    ...
                    <div className="flex justify-between items-start border-b-2 border-black pb-4 mb-4 pr-12">
                        <div>
                            <h4 className="font-black text-xl md:text-2xl tracking-tighter leading-none">PHRONT<span className="text-slate-500">.SYSTEMS</span></h4>
                            <div className="text-[8px] md:text-[9px] uppercase tracking-[0.2em] mt-1 font-bold">Resguardo de Activos v3.6</div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-y-2 md:gap-y-3 text-[10px] md:text-[11px] mb-6">
                        ...
                        <div className="text-right font-bold text-[9px] md:text-[11px]">{formatDate(bet.created_at)}</div>
                    </div>

                    {/* Main Numbers & Stats */}
                    <div className={`border-y-2 border-dashed border-slate-400 py-4 md:py-6 mb-6 flex flex-col items-center gap-4 transition-colors duration-500 ${isWin ? 'bg-green-100/50' : ''}`}>
                        <div className="text-center w-full">
                            <div className="text-[8px] md:text-[9px] text-slate-500 uppercase font-black mb-1">Vector Seleccionado</div>
                            <div className={`text-5xl md:text-7xl font-black tracking-tighter leading-none ${isWin ? 'text-green-600 drop-shadow-sm' : ''}`}>{bet.numbers}</div>
                        </div>
                        ...
                        <div className="grid grid-cols-3 w-full gap-2 px-1">
                            <div className="text-center border-r border-slate-300">
                                <div className="text-[7px] md:text-[8px] text-slate-500 uppercase font-black mb-1">Inversión</div>
                                <div className="text-sm md:text-lg font-black">{formatCurrency(bet.amount_bigint)}</div>
                            </div>
                            <div className="text-center border-r border-slate-300">
                                <div className="text-[7px] md:text-[8px] text-emerald-600 uppercase font-black mb-1">Premio Ganado</div>
                                <div className={`text-sm md:text-lg font-black ${isWin ? 'text-green-600 animate-pulse' : 'text-slate-700'}`}>
                                    {formatCurrency(totalPrize)}
                                </div>
                            </div>
                            <div className="text-center">
                                <div className="text-[7px] md:text-[8px] text-slate-500 uppercase font-black mb-1">Factor</div>
                                <div className={`text-[9px] md:text-[10px] font-black uppercase leading-tight ${isWin ? 'text-green-600' : 'text-slate-700'}`}>
                                    {multiplier}x
                                </div>
                            </div>
                        </div>
                    ...
                    {isWin ? (
                        <div className="mb-6 p-4 md:p-5 bg-black text-white rounded-2xl border-2 border-green-500 shadow-[0_5px_30px_rgba(34,197,94,0.4)] animate-pulse relative overflow-hidden group">
                            ...
                            <div className="text-[9px] md:text-[10px] text-green-400 uppercase font-black tracking-[0.25em] mb-1">LIQUIDACIÓN ACREDITADA</div>
                            <div className="text-2xl md:text-4xl font-black font-mono tracking-tighter text-white">
                                {formatCurrency(totalPrize)}
                            </div>
                            <div className="mt-2 pt-2 border-t border-white/10 flex items-center gap-2">
                                <div className="w-1.5 h-1.5 rounded-full bg-green-500"></div>
                                <span className="text-[8px] font-bold text-green-500 uppercase tracking-widest">Saldo disponible en bóveda</span>
                            </div>
                        </div>
                    ) : (
                    ...
                    <div className="text-[10px] flex flex-col md:flex-row justify-between items-end gap-4">
                        <div className="w-full md:w-auto">
                            ...
                            <div className={`font-black text-sm md:text-base px-3 py-1.5 rounded-lg border-2 text-center ${
                    ...
                    <div className="text-center text-[8px] md:text-[9px] font-mono tracking-[0.8em] mt-2 font-black">PHRONT_VERIFIED_TICKET</div>
                </div>
            ...
            <div className="text-center mt-6">
                <p className="text-white/40 text-[9px] md:text-[10px] font-mono uppercase tracking-[0.4em]">Protocolo Maestro de Seguridad de Activos</p>
            </div>
        </div>
    </div>,
    document.body
