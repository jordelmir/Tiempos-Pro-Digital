...
export default function WithdrawModal({
  ...
  const handleWithdraw = async () => {
    ...
    try {
      ...
      setTimeout(() => {
        onSuccess?.();
        onClose();
      }, 8000); // Extended auto-close time to read balance
    } catch (e: any) {
      setStatus('ERROR');
  ...
  const getTicketClass = () => {
    switch (status) {
      ...
      case 'CUTTING':
        return 'translate-y-2 opacity-100';
      case 'DROPPING':
        return 'translate-y-[150%] rotate-12 opacity-0 pointer-events-none transition-all duration-700 ease-in';
      case 'FINISHED':
        return 'hidden';
      default:
        return 'hidden';
      ...
      <div className="relative max-w-sm w-full mx-4 perspective-1000 max-h-[90vh] flex flex-col">
        <div className="absolute -top-8 left-1/2 -translate-x-1/2 w-[95%] z-10">
          <div className="h-10 bg-[#0a0a0a] rounded-t-xl shadow-[inset_0_5px_15px_black] border-x border-t border-white/10 flex items-end justify-center overflow-hidden relative">
            <div
              className={`absolute top-0 left-0 w-full h-full bg-cyber-orange blur-xl transition-all duration-300 ${status === 'PRINTING' ? 'opacity-40' : 'opacity-0'}`}
            ></div>
            <div className="w-[90%] h-1 bg-black rounded-full mb-2 relative z-20 shadow-[0_0_10px_black] overflow-hidden">
              <div
                className={`w-full h-full transition-all duration-300 ${status === 'PRINTING' ? 'bg-white shadow-[0_0_20px_white] opacity-100' : 'bg-cyber-orange opacity-20'}`}
              ></div>
            </div>
          </div>
        </div>
        ...
        <div
          className={`bg-[#0f0f12] border-2 rounded-2xl w-full relative z-20 overflow-hidden flex flex-col min-h-[400px] transition-all duration-200 ${status === 'INSUFFICIENT_FUNDS' ? 'border-red-600 shadow-[0_0_50px_rgba(255,0,0,0.5)] animate-[shake_0.4s_ease-in-out]' : 'border-cyber-orange shadow-[0_0_60px_rgba(255,95,0,0.15)]'}`}
        >
          <div className="absolute inset-0 opacity-5 pointer-events-none bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')]"></div>

          <div
            className={`p-6 border-b relative z-20 transition-colors duration-300 ${status === 'INSUFFICIENT_FUNDS' ? 'bg-red-950/50 border-red-500/50' : 'bg-[#050a14] border-white/5'}`}
          >
            <button
              onClick={handleClose}
            ...
            <div className="flex items-center gap-4">
              <div
                className={`w-12 h-12 rounded flex items-center justify-center shadow-neon-orange relative overflow-hidden group transition-all ${status === 'INSUFFICIENT_FUNDS' ? 'bg-red-600 text-white border border-white' : 'bg-cyber-orange/10 border border-cyber-orange/30'}`}
              >
                <i
                  className={`fas ${status === 'INSUFFICIENT_FUNDS' ? 'fa-ban' : 'fa-hand-holding-usd'} text-2xl ${status === 'INSUFFICIENT_FUNDS' ? 'text-white animate-pulse' : 'text-cyber-orange'} relative z-10`}
                ></i>
              </div>
              <div>
                <h3
                  className={`font-display font-bold uppercase tracking-wider transition-colors ${status === 'INSUFFICIENT_FUNDS' ? 'text-red-500' : 'text-white'}`}
                >
                  {status === 'INSUFFICIENT_FUNDS' ? 'ACCESO DENEGADO' : 'Retiro de Fondos'}
                </h3>
                <div className="text-[9px] font-mono uppercase tracking-widest flex items-center gap-1 text-cyber-orange">
                  {status}
                </div>
              </div>
            </div>
          ...
          <div className="p-6 relative flex-1 flex flex-col overflow-y-auto custom-scrollbar">
            {(status === 'IDLE' ||
              ...
              <div
                className={`transition-all duration-500 ${status === 'PROCESSING' ? 'opacity-50 scale-95 pointer-events-none' : 'opacity-100'}`}
              >
                <div
                  className={`bg-black border rounded-lg p-4 mb-6 relative overflow-hidden group transition-colors duration-300 ${status === 'INSUFFICIENT_FUNDS' ? 'border-red-500/50' : 'border-slate-800'}`}
                >
                  ...
                  <div className="text-right mt-2 relative z-10">
                    ...
                    <span
                      className={`font-mono font-bold text-glow-sm transition-colors ${status === 'INSUFFICIENT_FUNDS' ? 'text-red-500' : 'text-white'}`}
                    >
                ...
                <div className="mb-6 relative group/input">
                  <label className="block text-[10px] font-mono font-bold text-cyber-orange mb-2 uppercase tracking-wider ml-1">
                    Monto a Retirar
                  </label>
                  <div className="relative">
                    <input
                      ...
                      autoFocus
                    />
                    <span className="absolute right-2 bottom-4 text-slate-600 font-bold text-xs">
                      CRC
                    </span>
                  </div>
                </div>
                ...
                >
                  {status === 'SCANNING' || status === 'PROCESSING' ? (
                    ...
                    <i className="fas fa-print group-hover/btn:animate-pulse"></i>
                  )}
                  {status === 'SCANNING'
                    ? 'VERIFICANDO...'
                    : status === 'PROCESSING'
            ...
            {status === 'FINISHED' && (
              <div className="absolute inset-0 flex flex-col items-center justify-center z-40 animate-in zoom-in-95 duration-500 bg-black/90">
                ...
                <h4 className="text-3xl font-display font-black text-white uppercase italic tracking-wider mb-1 drop-shadow-lg">
                  Transacción <br />
                  <span className="text-cyber-orange text-glow">Finalizada</span>
                </h4>
                ...
                <div className="mt-6 bg-[#050a14] border border-cyber-orange/30 p-4 rounded-xl text-center min-w-[200px] relative overflow-hidden group">
                  ...
                  <div className="text-[9px] font-mono text-slate-500 uppercase tracking-widest mb-1 relative z-10">
                    Saldo Restante
                  </div>
            ...
            {['PRINTING', 'CUTTING', 'DROPPING'].includes(status) && (
              ...
              >
                <div
                  ...
                  style={{
                    clipPath: 'polygon(0 0, 100% 0, 100% 98%, 0 98%)',
